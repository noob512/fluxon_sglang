#!/usr/bin/env python3
"""
Fluxon Quick Start - unified installed entrypoint.

Usage:
  python3 -m fluxon_py.quick_start --mode kv [options]
  python3 -m fluxon_py.quick_start --mode mq [options]
  python3 -m fluxon_py.quick_start --mode fs [options]
"""
...
from fluxon_py.quick_start import _DEFAULT_EXPORT_NAME, _validate_export_name
...
def _downloaded_binary_dir() -> Path:
    configured = os.environ.get("FLUXON_QUICK_START_BIN_DIR")
    if configured:
        return Path(configured).expanduser()
    cache_root = os.environ.get("XDG_CACHE_HOME")
    if cache_root:
        return Path(cache_root).expanduser() / "fluxon" / "quick_start" / "bin"
    return Path.home() / ".cache" / "fluxon" / "quick_start" / "bin"


def _auto_download_binary(name: str) -> str:
    """Download a missing runtime binary into the user's cache directory."""
    import tarfile
    import urllib.request
    ...
    bin_dir = _downloaded_binary_dir()
...
def _find_binary(name: str) -> str:
    """Find a packaged, source-tree, PATH, or cached binary."""
    local_candidates = [
        SCRIPT_DIR / "bin" / name,
        REPO_ROOT / "examples" / "fluxon_quick_start" / "bin" / name,
    ]
    ...
    if found:
        return found
    cached = _downloaded_binary_dir() / name
    if cached.exists():
        return str(cached)
...
def _gen_fs_config(
    ...
    *,
    ...
    greptime_base_url: Optional[str] = None,
) -> Dict[str, Any]:
    export_name = _validate_export_name(export_name)
    master_config = deepcopy(kv_master_config)
    owner_config = deepcopy(kv_owner_config)
    master_cluster = master_config.get("cluster_name")
    if not isinstance(master_cluster, str) or not master_cluster:
        raise ValueError("kv_master_config.cluster_name must be a non-empty string")
    owner_spec = owner_config.get("fluxonkv_spec")
    if not isinstance(owner_spec, dict):
        raise ValueError("kv_owner_config.fluxonkv_spec must be a dict")
    owner_cluster = owner_spec.get("cluster_name")
    if not isinstance(owner_cluster, str) or not owner_cluster:
        raise ValueError(
            "kv_owner_config.fluxonkv_spec.cluster_name must be a non-empty string"
        )
    ...
    master_endpoints = master_config.get("etcd_endpoints")
    owner_endpoints = owner_spec.get("etcd_addresses")
    ...
    shm = owner_spec.get("share_mem_path")
    if not isinstance(shm, str) or not shm:
        raise ValueError(
            "kv_owner_config.fluxonkv_spec.share_mem_path must be a non-empty string"
        )
    cluster = master_cluster
    remote_root_dir = str(fs_root if fs_root is not None else workdir / "fs_remote_root")
    access_db_path = str(workdir / "fs_master" / "access.db")
    export_cache_max_bytes = 1073741824
    if greptime_base_url is None:
        greptime_base_url = f"http://127.0.0.1:{greptime_http_port}"
    greptime_base_url = greptime_base_url.rstrip("/")
    panel_public_base_url = ""
    if panel_port:
        panel_public_base_url = _build_panel_urls(panel_port=panel_port, path="")[0]
    cfg: Dict[str, Any] = {
        "master": master_config,
        "kvclient": owner_config,
        "fs_master": {
            "kvclient": {
...
def _wait_for_http_ready(url: str, label: str, timeout: int = 30) -> None:
    deadline = time.time() + timeout
    while time.time() < deadline:
        try:
...
def _http_endpoint_host_port(base_url: str, *, label: str) -> tuple[str, int]:
    parsed = urllib.parse.urlparse(base_url)
    if parsed.scheme not in {"http", "https"} or parsed.hostname is None:
        raise ValueError(f"{label} must be an http(s) URL with an explicit port")
    try:
        port = parsed.port
...
def _start_cluster_infra(
    *,
    ...
    greptime_port: int,
    external_middleware: bool = False,
    greptime_base_url: Optional[str] = None,
) -> None:
    if external_middleware:
        master_endpoints = cfg["master"].get("etcd_endpoints")
        if not isinstance(master_endpoints, list) or len(master_endpoints) != 1:
            raise ValueError("external middleware requires exactly one etcd endpoint")
        etcd_ep = str(master_endpoints[0])
    else:
    ...
    if external_middleware:
        if greptime_base_url is None:
            greptime_base_url = f"http://127.0.0.1:{greptime_port}"
        greptime_host, greptime_ready_port = _http_endpoint_host_port(
            greptime_base_url,
            label="greptime_base_url",
        )
        etcd_base_url = etcd_ep if "://" in etcd_ep else f"http://{etcd_ep}"
        print(f"[quick_start] using external greptime at {greptime_base_url}")
        _wait_for_tcp(greptime_host, greptime_ready_port, "greptime", timeout=30)
        print(f"[quick_start] using external etcd at {etcd_ep}")
        _wait_for_http_ready(f"{etcd_base_url.rstrip('/')}/health", "etcd", timeout=30)
    else:
...
def _prepare_fs_root(fs_root: Path, *, create_sample: bool) -> None:
...
def mode_fs(
    ...
    *,
    ...
    workdir = args.workdir
    greptime_port = args.greptime_http_port
    explicit_fs_root = args.fs_root is not None
    fs_root = args.fs_root.resolve() if explicit_fs_root else workdir / "fs_remote_root"
    cfg = _gen_fs_config(
        args.panel_port,
        greptime_port,
        workdir,
        ...
        export_name=(
            ...
            else _DEFAULT_EXPORT_NAME
        ),
        greptime_base_url=args.greptime_base_url,
    )

    # Only the built-in demo directory receives sample data. Explicit exports are left untouched.
    _prepare_fs_root(fs_root, create_sample=not explicit_fs_root)
    ...
    _start_cluster_infra(
        ...
        etcd_client_port=args.etcd_client_port,
        greptime_port=greptime_port,
        external_middleware=args.external_middleware,
        greptime_base_url=args.greptime_base_url,
    )
    ...
    s3_urls = _build_panel_urls(panel_port=args.panel_port, path="/fs_s3")
    ui_urls = _build_panel_urls(panel_port=args.panel_port, path="/fs_s3/ui/")
    if ui_urls:
        print()
        _print_panel_urls(label="FS S3 endpoint", urls=s3_urls)
        _print_panel_urls(label="FS Web UI", urls=ui_urls)
        print("  Initial Web UI credentials: admin / admin")
        print(
            "  First sign-in requires changing both the username and password "
            "before S3 access is enabled."
        )
    print(f"  FS remote root: {fs_root}")
    print(f"  KV master log dir: {cfg['master']['log_dir']}")
    owner_spec = cfg["kvclient"]["fluxonkv_spec"]
    print(f"  KV owner shared path: {owner_spec['share_mem_path']}")
    print(
        "  KV owner large-file paths: "
        + ", ".join(str(path) for path in owner_spec["large_file_paths"])
    )

    if args.serve:
        _run_service_until_stopped()
    else:
        _run_fs_interactive(fs_root)


def _run_service_until_stopped() -> None:
    """Keep quick-start children alive until SIGINT/SIGTERM or a child exits."""

    shutdown_requested = threading.Event()

    def _on_shutdown(reason: str) -> None:
        print(f"\n[quick_start] stopping FS service: {reason}")
        shutdown_requested.set()

    restore_signal_listener = register_ctrlc_callback(
        _on_shutdown,
        thread_name="qs-fs-service-signal",
    )
    print("\n=== Fluxon FS Service ===")
    ...
    try:
        while not shutdown_requested.wait(0.5):
            for child in _children:
                ...
                if rc is not None:
                    raise SystemExit(
                        f"[quick_start] child process exited unexpectedly: pid={child.pid} rc={rc}"
                    )
    finally:
        restore_signal_listener()
    ...
    *,
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent("""\
            Examples:
              python3 -m fluxon_py.quick_start --mode kv --etcd-client-port 12379 --master-p2p-port 31000 --greptime-http-port 14000 --kv-http-port 8083
              python3 -m fluxon_py.quick_start --mode mq --etcd-client-port 37379 --kv-master-port 34200 --greptime-http-port 14000 --panel-port 18080
              python3 -m fluxon_py.quick_start --mode fs --serve --fs-root /data --workdir /state --share-mem-path /dev/shm/fluxon-s3 --etcd-client-port 22379 --kv-master-port 25100 --greptime-http-port 24000 --panel-port 26180
        """),
    )
    ...
    parser.add_argument(
        "--external-middleware",
        action="store_true",
        help="Reuse an existing etcd and GreptimeDB instead of starting bundled binaries",
    )
    parser.add_argument(
        "--greptime-base-url",
        default=None,
        help="External GreptimeDB HTTP base URL, for example http://127.0.0.1:24000",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="KV owner shared-memory root (required in FS mode)",
    )
    parser.add_argument(
        "--fs-root",
        type=Path,
        default=None,
        help="Existing directory to export in FS mode",
    )
    parser.add_argument(
        "--export-name",
        default=None,
        help=(
            "FS export and S3 bucket name "
            f"(default: {_DEFAULT_EXPORT_NAME})"
        ),
    )
    parser.add_argument(
        "--serve",
        action="store_true",
        help="Run FS mode as a long-lived service without an interactive shell",
    )
    ...
    if args.mode == "fs" and args.panel_port == 0:
        raise ValueError("--panel-port is required in fs mode")
    if args.serve and args.mode != "fs":
        raise ValueError("--serve is supported only in fs mode")
    if args.fs_root is not None and args.mode != "fs":
        raise ValueError("--fs-root is supported only in fs mode")
    if args.export_name is not None and args.mode != "fs":
        raise ValueError("--export-name is supported only in fs mode")
    if args.share_mem_path is not None and args.mode != "fs":
        raise ValueError("--share-mem-path is supported only in fs mode")
    if args.external_middleware and args.mode != "fs":
        raise ValueError("--external-middleware is supported only in fs mode")
    if args.greptime_base_url is not None and not args.external_middleware:
        raise ValueError("--greptime-base-url requires --external-middleware")
    ...
    if args.mode == "fs":
        has_master = fs_kv_master_config is not None
        has_owner = fs_kv_owner_config is not None
        if has_master != has_owner:
            raise ValueError(
                "FS mode requires both fs_kv_master_config and fs_kv_owner_config"
            )
        if not has_master:
            if args.share_mem_path is None:
                raise ValueError("--share-mem-path is required in fs mode")
            etcd_ep = f"127.0.0.1:{args.etcd_client_port}"
            ...
            fs_kv_master_config = {
                ...
                "cluster_name": cluster,
                "instance_key": "qs_master",
                "port": args.kv_master_port,
                "log_dir": str(args.workdir / "kv-master" / "log"),
                "monitoring": _monitoring_block(args.greptime_http_port),
            }
            fs_kv_owner_config = {
                "instance_key": "qs_kvclient",
                "contribute_to_cluster_pool_size": {"dram": 1073741824, "vram": {}},
                "fluxonkv_spec": {
                    "etcd_addresses": [etcd_ep],
                    "cluster_name": cluster,
                    "share_mem_path": str(args.share_mem_path.resolve()),
                    "sub_cluster": "default",
                    "large_file_paths": [str(args.workdir / "kv-owner" / "large")],
                },
            }
    ...
    if args.mode == "kv":
        ...
        assert fs_kv_master_config is not None
        assert fs_kv_owner_config is not None
        mode_fs(
            args,
            kv_master_config=fs_kv_master_config,
            kv_owner_config=fs_kv_owner_config,
        )
