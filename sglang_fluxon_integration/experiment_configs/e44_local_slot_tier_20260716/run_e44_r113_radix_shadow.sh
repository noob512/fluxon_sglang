...
run_id="${1:?usage: $0 RUN_ID [eager_all|enddepth288]}"
admission="${2:-eager_all}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

case "$admission" in
  eager_all)
    variant=tier1_independent_005_netobs_r113_eager_all_radix_shadow_retain
    ;;
  enddepth288)
    variant=tier1_independent_005_netobs_enddepth288_r113_radix_shadow_retain
    ;;
  *) echo "admission must be eager_all or enddepth288" >&2; exit 2 ;;
esac

exec env \
  E44_CAPACITY_VARIANT="$variant" \
  E44_CAPACITY_CPU_PORT=31505 \
  E44_CAPACITY_CPU_VENV=/tmp/fluxon_runtime/venv-r113-radix-shadow-cpu-py311-20260803 \
