...
_DEFAULT_BATCH_EXISTS_PIN_TTL_MS = 30_000
_BATCH_EXISTS_PIN_TTL_ENV = "SGLANG_FLUXON_BATCH_EXISTS_PIN_TTL_MS"
...
def _maybe_add_side_by_side_fluxon_repo() -> None:
    for parent in Path(__file__).resolve().parents:
        if (parent / "fluxon_py").is_dir():
...
def _import_fluxon_symbols() -> tuple[Any, Any, Any, Any]:
    try:
        ...
        if exc.name != "fluxon_py":
            raise
        _maybe_add_side_by_side_fluxon_repo()
        fluxon_mod = importlib.import_module("fluxon_py")
...
class HiCacheFluxon(HiCacheStorage):
    """Fluxon-backed HiCache storage using CPU DLPack payloads."""

    def __init__(self, storage_config: HiCacheStorageConfig):
        self.storage_config = storage_config
        self.extra_config = dict(storage_config.extra_config or {})
        self.extra_backend_tag = self.extra_config.get("extra_backend_tag")
        key_prefix = self.extra_config.get("key_prefix")
        if key_prefix is None:
            key_prefix = "sglang_hicache"
        ...
        self._warm_condition = threading.Condition(self._warm_lock)
        self._enable_warm_get = _extra_config_bool(
            self.extra_config, "enable_warm_get", True
        )
        self._warm_limit = max(1, int(self.extra_config.get("warmup_pending_limit", 4096)))
        self._warm_submit_limit = max(
            1,
            int(
                self.extra_config.get(
                    "warmup_submit_limit", min(128, self._warm_limit)
                )
            ),
        )
        self._warm_drain_budget = max(
            0, int(self.extra_config.get("warmup_drain_budget", 64))
        )
        self._warm_peak_pending = 0
        self._warm_auto_drained = 0
        self._warm_auto_drain_failed = 0
        self._batch_concurrency = max(1, int(self.extra_config.get("batch_concurrency", 128)))
        pin_ttl_config = self.extra_config.get(
            "batch_exists_pin_ttl_ms",
            self.extra_config.get(
                "batch_exist_pin_ttl_ms",
                os.environ.get(
                    _BATCH_EXISTS_PIN_TTL_ENV, _DEFAULT_BATCH_EXISTS_PIN_TTL_MS
                ),
            ),
        )
        self._batch_exists_pin_ttl_ms = _optional_positive_int(pin_ttl_config)
        self._warm_batch_executor = ThreadPoolExecutor(
            max_workers=max(
                1,
                int(self.extra_config.get("warmup_batch_workers", 1)),
            ),
        ...
        self._replica_task_config = _parse_replica_task_config(self.extra_config)
        ...
        config_path = (
            self.extra_config.get("config_path")
            or envs.SGLANG_REMOTE_PAGE_CACHE_FLUXON_CONFIG_PATH.get()
        )
        if not config_path:
            raise RuntimeError(
                "Fluxon HiCache backend requires extra_config['config_path'] "
                "or SGLANG_REMOTE_PAGE_CACHE_FLUXON_CONFIG_PATH."
        )
        ...
        hicache_write_policy = str(
            self.extra_config.get("hicache_write_policy", "")
        ).strip()
        update_master_router = hicache_write_policy == "write_through"
    ...
    def _put_opts_for_replica_task_mask(
        ...
        return _build_put_optional_args(
            ...
            content_depths=(
                None if content_depths is None else list(content_depths)
            ),
    ...
    def _submit_warm_get(self, storage_key: str) -> bool:
        with self._warm_lock:
            ...
            self._warm_peak_pending = max(
                self._warm_peak_pending,
    ...
    def _submit_warm_get_batch(
        self,
        storage_keys: list[str],
        *,
        release_on_complete: bool = False,
    ) -> list[str]:
        ...
        accepted: list[str] = []
        accepted_entries: list[tuple[str, Any]] = []
        with self._warm_lock:
            try:
                if not self._enable_warm_get:
                    accepted = []
                    ...
                    for batch_index, storage_key in enumerate(selected):
                        ...
                        if storage_key in self._warm_futures:
                            continue
                        warm_future = _FluxonBatchWarmFuture(
                            batch_future,
                            batch_index,
                            storage_key,
                        )
                        self._warm_futures[storage_key] = warm_future
                        accepted.append(storage_key)
                        accepted_entries.append((storage_key, warm_future))
            finally:
                for storage_key in selected:
                    self._warm_inflight.discard(storage_key)
                self._warm_condition.notify_all()
        if release_on_complete and accepted_entries:
            tracked_entries = tuple(accepted_entries)

            def _release_completed_batch(_batch_future: Future) -> None:
                self._drain_completed_warm_entries(
                    tracked_entries,
                    reason="batch_terminal",
                )

            batch_future.add_done_callback(_release_completed_batch)
        return accepted

    def _drain_completed_warm_entries(
        self,
        ...
        *,
        ...
        with self._warm_lock:
            for storage_key, expected_future in entries:
                current_future = self._warm_futures.get(storage_key)
                if current_future is not expected_future:
                    continue
                is_waiting = getattr(current_future, "is_waiting", None)
                if is_waiting is not None and bool(is_waiting()):
                    continue
                self._warm_futures.pop(storage_key, None)
                ready_items.append((storage_key, current_future))
        ...
        for storage_key, future in ready_items:
            try:
                ...
                failed += 1
                logger.debug(
                    "Fluxon warm get auto-drain raised for %s: %s",
                    storage_key,
                    exc,
                )
                continue
            if not hasattr(wait_result, "is_ok"):
                failed += 1
                logger.debug(
                    "Fluxon warm get auto-drain returned invalid result for %s: %s",
                    storage_key,
                    wait_result,
                )
                continue
            try:
                if wait_result.is_ok():
                    # fluxon_py.Result is move-like and enforces explicit
                    # consumption in __del__. Merely checking is_ok() leaves
                    # the MemHolder live; unwrap and immediately drop it so
                    # the owner-local reader pin is actually released.
                    ...
                    logger.debug(
                        "Fluxon warm get auto-drain failed for %s: %s",
                        storage_key,
                        error,
                    )
            except Exception as exc:
                failed += 1
                logger.warning(
                    "Fluxon warm get auto-drain result consumption raised "
                    "for %s: %s",
                    storage_key,
                    exc,
                )

        drained = len(ready_items)
        if drained:
            with self._warm_lock:
                self._warm_auto_drained += drained
                self._warm_auto_drain_failed += failed
                pending = len(self._warm_futures) + len(self._warm_inflight)
                auto_drained = self._warm_auto_drained
                auto_failed = self._warm_auto_drain_failed
                peak_pending = self._warm_peak_pending
            logger.info(
                "Fluxon host prefetch holders released: reason=%s drained=%d "
                "failed=%d pending=%d auto_drained=%d auto_failed=%d "
                "peak_pending=%d",
                reason,
                drained,
                failed,
    ...
    def _drain_ready_warm_futures(self, budget: Optional[int] = None) -> int:
        ...
        with self._warm_lock:
            for storage_key, future in list(self._warm_futures.items()):
                if len(ready_items) >= budget:
                    break
                is_waiting = getattr(future, "is_waiting", None)
                if is_waiting is not None and is_waiting():
                    continue
                ready_items.append((storage_key, future))
        return self._drain_completed_warm_entries(
            ready_items,
    ...
    def set_warm_get_enabled(
        ...
        if not enabled:
            # Do not erase an unfinished flight: a late Plan could otherwise
            # start GDR while the owner-local payload transfer is still live.
            # Completed entries are safe to consume/drop; unfinished entries
            # remain discoverable until their normal waiter/callback drains.
    ...
    def _warm_keys(self, storage_keys: List[str]) -> int:
        ...
        submitted_keys = self._submit_warm_get_batch(unique_keys)
    ...
    def prefetch_to_host(
        ...
        """Start an owner-local DRAM warmup and return the tracked input keys.

        The returned keys are the only keys whose warm futures a later
        ``finish_prefetch_to_host`` call must join.  Keys beyond the bounded
        host lane are deliberately left untouched so the execution-time
        materializer may bind them directly to GPU destinations without
        duplicating an in-flight remote transfer.
        """
        if not keys or not self._enable_warm_get:
            return ()
        limit = len(keys) if max_keys is None else max(0, int(max_keys))
        if limit == 0:
            return ()
        input_keys = list(keys[:limit])
        storage_keys = [self._store_key(key, component_name) for key in input_keys]
        with self._warm_lock:
            tracked_storage_keys = {
                storage_key
                for storage_key in storage_keys
                if storage_key in self._warm_futures
                or storage_key in self._warm_inflight
            }
        submitted_storage_keys = self._submit_warm_get_batch(
            storage_keys,
        ...
        tracked = tuple(
            key
            for key, storage_key in zip(input_keys, storage_keys)
            if storage_key in tracked_storage_keys
        )
        logger.info(
            "Fluxon host prefetch submitted: requested=%d tracked=%d "
            "max_keys=%d pending=%d pending_limit=%d peak_pending=%d "
            "release_on_complete=%s auto_drained=%d component=%s",
            len(keys),
            len(tracked),
            limit,
            pending,
    ...
    def finish_prefetch_to_host(
        self,
        keys: Sequence[str],
        component_name: Optional[Any] = None,
    ) -> tuple[list[Any], dict[str, Any]]:
        """Join exactly the early host lane and retain successful holders.

        Callers keep the returned holders alive until the execution-time
        GetPlan has acquired fresh owner-local pins.  A failed/absent warm
        future is terminal before this method returns, so re-planning that key
        cannot race a second payload transfer.
        """
        start = time.perf_counter()
        keepalives: list[Any] = []
        waited = 0
        ...
        failed = 0
        for key in keys:
            storage_key = self._store_key(key, component_name)
            ...
            if hasattr(result, "is_ok") and result.is_ok():
                ...
                error = (
                    result.unwrap_error()
                    if hasattr(result, "unwrap_error")
                    else result
                )
                logger.warning(
                    "Fluxon host prefetch failed before materialization: key=%s error=%s",
                    storage_key,
                    error,
                )
        stats = {
            "requested": len(keys),
            "ready": len(keepalives),
            "waited": waited,
            "already_terminal": already_terminal,
            "missing": missing,
            # A key returned by prefetch_to_host was admitted to an existing
            # or newly submitted flight. The only normal removers wait for a
            # terminal result first, so a missing entry here means its holder
            # was already drained/consumed and is safe to re-plan.
            "terminal_drained": missing,
            "failed": failed,
            "wait_ms": (time.perf_counter() - start) * 1000.0,
        }
        logger.info(
            "Fluxon host prefetch joined: requested=%d ready=%d waited=%d "
            "already_terminal=%d terminal_drained=%d failed=%d "
            "wait_ms=%.3f component=%s",
            stats["requested"],
            stats["ready"],
            stats["waited"],
            stats["already_terminal"],
            stats["terminal_drained"],
            stats["failed"],
            stats["wait_ms"],
            component_name,
        )
        return keepalives, stats

    def _take_warm_future(self, storage_key: str) -> Any | None:
        with self._warm_condition:
            self._warm_condition.wait_for(
                lambda: storage_key not in self._warm_inflight
            )
    ...
    def local_fast_put_start(
        ...
        logger.info(
            ...
            "content_depth_min=%s content_depth_max=%s atomic_group_lens=%s duration_ms=%.3f",
            len(storage_keys),
            value_len,
            component_name,
            sum(1 for item in admission_mask if item),
    ...
    def _batch_io_v2(
        ...
        for transfer in transfers:
            ...
            fragment_ptrs, fragment_sizes, fragments_per_key = (
                self._page_fragment_meta_from_host_indices(
                    host_pool,
                    host_indices,
                    len(keys),
                    f"{op_name}/{transfer.name}",
                )
            )
            storage_keys = [self._store_key(key, transfer.name) for key in keys]
            if op_name == "batch_get_v2":
                pool_results, pool_bytes = self._batch_get_fragment_ptrs_flat(
                    storage_keys,
                    fragment_ptrs,
                    fragment_sizes,
                    fragments_per_key,
                ...
                pool_results, put_bytes = self._batch_put_fragment_ptrs_flat(
                    storage_keys,
                    fragment_ptrs,
                    fragment_sizes,
                    fragments_per_key,
    ...
    def batch_get_v1(
        ...
        if host_indices.numel() != expected:
            logger.error(
                "Fluxon batch_get_v1 indices length mismatch: expected=%s got=%s",
                expected,
                host_indices.numel(),
            )
        ...
        fragment_ptrs, fragment_capacities, fragments_per_key = (
            self._page_fragment_meta_from_host_indices(
                self.mem_pool_host, host_indices, len(keys), "batch_get_v1"
            )
        )
        results, total_bytes = self._batch_get_fragment_ptrs_flat(
            storage_keys,
            fragment_ptrs,
            fragment_capacities,
            fragments_per_key,
    ...
    def batch_set_v1(
        ...
        if host_indices.numel() != expected:
            logger.error(
                "Fluxon batch_set_v1 indices length mismatch: expected=%s got=%s",
                expected,
                host_indices.numel(),
            )
            return [False] * len(keys)
        ...
        storage_keys = [self._store_key(key) for key in keys]
        start = time.perf_counter()
        fragment_ptrs, fragment_lens, fragments_per_key = self._page_fragment_meta_from_host_indices(
            self.mem_pool_host, host_indices, len(keys), "batch_set_v1"
        )
        results, total_bytes = self._batch_put_fragment_ptrs_flat(
            storage_keys,
            fragment_ptrs,
            fragment_lens,
            fragments_per_key,
