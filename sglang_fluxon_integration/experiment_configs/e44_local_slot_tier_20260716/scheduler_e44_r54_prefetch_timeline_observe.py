...
from sglang.srt.mem_cache.remote_page_controller import RemotePageController
...
class Scheduler(
    ...
    def __init__(
        ...
        self.remote_page_controller = RemotePageController(
            token_to_kv_pool_allocator=self.token_to_kv_pool_allocator,
            page_size=self.page_size,
        )
        if hasattr(self.tree_cache, "attach_remote_page_controller"):
            self.tree_cache.attach_remote_page_controller(self.remote_page_controller)
    ...
    def init_schedule_policy(self):
        ...
        self.policy = SchedulePolicy(
            ...
            self.schedule_low_priority_values_first,
            remote_page_controller=self.remote_page_controller,
        )
    ...
    def _prefetch_kvcache(self, req: Req):
        if self.enable_hicache_storage:
            ...
            self.remote_page_controller.register_match(req)
            last_host_node = req.last_host_node
            # The local radix selects a trustworthy hash anchor.  It must not
            # decide whether a shared L3 is queried: a different instance may
            # have published the suffix even when this node is neither root nor
            # locally backed.
            last_hash = last_host_node.get_last_hash_value()
            matched_len = len(req.prefix_indices) + req.host_hit_length
            prefetch_fill_ids = getattr(
                req, "full_untruncated_fill_ids", None
            )
            if prefetch_fill_ids is None:
                # The frozen experiment runtime rebuilds the complete
                # origin+output sequence in fill_ids immediately above, but
                # predates the full_untruncated_fill_ids compatibility field.
                prefetch_fill_ids = req.fill_ids
            match_end = req._compute_max_prefix_len(len(prefetch_fill_ids))
            new_input_tokens = prefetch_fill_ids[matched_len:match_end]
        ...
        for queue_position, req in enumerate(self.waiting_queue):
            ...
            if self.enable_hicache_storage:
                ...
                if not prefetch_done:
                    ...
                    continue
                # Keep the completed L3 amount across the second radix match.
                # Some compatible runtimes reset transient request hit fields
                # inside init_next_round_input().
                storage_hit_length = self.tree_cache.pop_prefetch_loaded_tokens(
                    req.rid
                )

            req.init_next_round_input(self.tree_cache)
            if self.enable_hicache_storage:
                req.storage_hit_length = storage_hit_length
            self.remote_page_controller.register_match(req)
