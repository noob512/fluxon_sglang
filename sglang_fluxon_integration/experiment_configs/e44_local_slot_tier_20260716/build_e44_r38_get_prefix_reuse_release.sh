#!/usr/bin/env bash
set -euo pipefail

workspace=/mnt/ceph/mjq/push_sglang
repo="${E44_BUILD_SOURCE_REPO:-$workspace/Fluxon}"
helpers="$workspace/experiment_configs/e16ay_get_target_pressure_shrink_20260713"
experiment="$workspace/experiment_configs/e44_local_slot_tier_20260716"
sdk="${E44_CLOSED_SDK_SOURCE:-/mnt/ceph/zyc/fluxon_closed/fluxon_release_e16bk37_batch_append_20260715/closed_sdk}"
base="${E44_RELEASE_BASE_SOURCE:-$workspace/Fluxon/fluxon_release_e16bk30_dead_protection_rollback_20260715}"
seed=/mnt/nvme0/mjq_open_fluxon_pack_env/open_fluxon/projects/511ab2c2fcc9c292dafc01629d47042763a583a8c0b0518bb8c3944465a6ffab/substrates/manylinux_container/target-caches/pack_release_tcp_thread_closed_sdk/134b3060fc2aa095473846a39de8933308054baece1ffec1e6a502867dad0368
target=/mnt/nvme0/mjq_build/push_sglang_fluxon_target
rootfs=/mnt/nvme0/mjq_build/fluxon_pypack_rootfs
release="${E44_RELEASE_DIR:-/mnt/nvme0/mjq_build/fluxon_e44_r38_get_prefix_reuse_20260720}"
source_provenance_mode="${E44_SOURCE_PROVENANCE_MODE:-git}"

case "$source_provenance_mode" in
  git | content_only) ;;
  *)
    echo "E44_SOURCE_PROVENANCE_MODE must be git or content_only" >&2
    exit 2
    ;;
esac

test "$(findmnt -n -o SOURCE -T "$target")" = /dev/nvme0n1p3
test -d "$rootfs"
test "$(findmnt -n -o SOURCE -T "$rootfs")" = /dev/nvme0n1p3
test "$(findmnt -n -o SOURCE -T "$(dirname "$release")")" = /dev/nvme0n1p3
ulimit -n 65535
mkdir -p "$target"
rm -rf -- "$release"
mkdir -p "$release"

if [ ! -f "$target/.seed_complete" ]; then
  rsync -rltL --no-perms --no-owner --no-group \
    "$seed/.rustc_info.json" \
    "$seed/cxxpacked" \
    "$seed/native_runtime" \
    "$seed/release" \
    "$seed/vendor_runtime" \
    "$target/"
  touch "$target/.seed_complete"
fi
if [ ! -f "$target/.tool_seed_complete" ]; then
  rsync -rltL --no-perms --no-owner --no-group \
    "$seed/dagviz" \
    "$seed/downloads" \
    "$seed/maturin" \
    "$seed/meson-0.64.0" \
    "$seed/target_cache_manifest.json" \
    "$target/"
  touch "$target/.tool_seed_complete"
fi

export CARGO_TARGET_DIR="$target"
export FLUXON_CARGO_TARGET_CACHE="$target"
export FLUXON_PYPACK_ROOTFS="$rootfs"
export FLUXON_RELEASE_DIR="$release"
export FLUXON_RELEASE_BASE="$base"
export FLUXON_BUILD_CLOSED_SDK_SOURCE="$sdk"
export FLUXON_RELEASE_CLOSED_SDK_SOURCE="$sdk"
export FLUXON_RELEASE_LINK_IMMUTABLE_ASSETS=0
export CARGO_BUILD_JOBS=1

PYTHONDONTWRITEBYTECODE=1 python3 -B \
  "$experiment/validate_e44_r38_get_prefix_reuse.py" \
  "$experiment/unified_radix_cache_e44_r38_get_prefix_reuse.py" \
  "$experiment/hicache_fluxon_e44_r38_get_prefix_reuse.py"

bash "$helpers/build_e16ay_release.sh"
bash "$helpers/finalize_e16ay_release.sh"

install -m 0644 \
  "$experiment/sglang_r12_metadata_host/memory_pool_host.py" \
  "$release/memory_pool_host_fluxon_metadata_only.py"
install -m 0644 \
  "$repo/fluxon_rs/fluxon_kv/src/external_client_api/delete_ack_batch.rs" \
  "$release/source_external_delete_ack_batch.rs"
install -m 0644 \
  "$repo/fluxon_rs/fluxon_kv/src/client_kv_api/external_api.rs" \
  "$release/source_client_external_api.rs"
install -m 0644 \
  "$repo/fluxon_rs/fluxon_kv/src/client_kv_api/get.rs" \
  "$release/source_client_get.rs"
install -m 0644 \
  "$repo/fluxon_rs/fluxon_kv/src/client_kv_api/msg_pack.rs" \
  "$release/source_client_msg_pack.rs"
install -m 0644 \
  "$repo/fluxon_rs/fluxon_kv/src/client_kv_api/mod.rs" \
  "$release/source_client_mod.rs"
install -m 0644 \
  "$repo/fluxon_rs/fluxon_kv/src/client_kv_api/reclaim.rs" \
  "$release/source_client_reclaim.rs"
install -m 0644 \
  "$repo/fluxon_rs/fluxon_kv/src/client_kv_api/put.rs" \
  "$release/source_client_put.rs"
install -m 0644 \
  "$repo/fluxon_rs/fluxon_kv/src/master_kv_router/mod.rs" \
  "$release/source_master_mod.rs"
install -m 0644 \
  "$repo/fluxon_rs/fluxon_kv/src/master_kv_router/msg_pack.rs" \
  "$release/source_master_msg_pack.rs"
install -m 0644 \
  "$repo/fluxon_rs/fluxon_kv/src/master_kv_router/put.rs" \
  "$release/source_master_put.rs"
install -m 0644 \
  "$repo/fluxon_rs/fluxon_kv/src/master_kv_router/reclaim.rs" \
  "$release/source_master_reclaim.rs"
install -m 0644 \
  "$repo/fluxon_rs/fluxon_kv/src/external_client_api/mod.rs" \
  "$release/source_external_client_mod.rs"
install -m 0644 \
  "$repo/fluxon_rs/fluxon_pyo3/src/lib.rs" \
  "$release/source_fluxon_pyo3_lib.rs"
install -m 0644 \
  "$repo/fluxon_py/kvclient/kvclient_interface.py" \
  "$release/source_python_kvclient_interface.py"
install -m 0644 \
  "$repo/fluxon_py/kvclient/fluxon.py" \
  "$release/source_python_fluxon.py"
install -m 0644 \
  "$repo/fluxon_doc_cn/design/sglang_fluxon_kv集成设计.md" \
  "$release/source_sglang_fluxon_kv_design_cn.md"
install -m 0644 \
  "$experiment/unified_radix_cache_e44_r38_get_prefix_reuse.py" \
  "$release/sglang_unified_radix_cache_r38.py"
install -m 0644 \
  "$experiment/hicache_fluxon_e44_r38_get_prefix_reuse.py" \
  "$release/sglang_hicache_fluxon_r38.py"
install -m 0755 \
  "$experiment/validate_e44_r38_get_prefix_reuse.py" \
  "$release/validate_e44_r38_get_prefix_reuse.py"
install -m 0755 \
  "$experiment/smoke_e44_r38_hicache_adapter.py" \
  "$release/smoke_e44_r38_hicache_adapter.py"
install -m 0755 \
  "$experiment/analyze_e44_r39_get_ready_breakdown.py" \
  "$release/analyze_e44_r39_get_ready_breakdown.py"

grep -F 'external Get start lifecycle:' "$release/source_client_external_api.rs" >/dev/null
grep -F 'external Get consume lifecycle:' "$release/source_client_external_api.rs" >/dev/null
grep -F 'external Get finish lifecycle:' "$release/source_client_get.rs" >/dev/null
PYTHONDONTWRITEBYTECODE=1 python3 -B \
  "$release/analyze_e44_r39_get_ready_breakdown.py" --self-test >/dev/null

if [ "$source_provenance_mode" = git ]; then
  (
    cd "$repo"
    git rev-parse HEAD > "$release/source_head.txt"
    git status --short > "$release/source_status.txt"
    git diff --binary > "$release/source_worktree.diff"
  )
else
  python3 - "$repo" "$release/source_head.txt" "${GIT_COMMIT_HASH:-}" <<'PY'
from pathlib import Path
import re
import sys

repo = Path(sys.argv[1])
output = Path(sys.argv[2])
content_identity = sys.argv[3]
git_entry = repo / ".git"
if git_entry.is_file():
    prefix = "gitdir: "
    value = git_entry.read_text(encoding="utf-8").strip()
    if not value.startswith(prefix):
        raise SystemExit("invalid linked-worktree .git pointer")
    git_dir = Path(value[len(prefix):])
elif git_entry.is_dir():
    git_dir = git_entry
else:
    git_dir = None
if git_dir is None:
    if re.fullmatch(r"[0-9a-f]{40}", content_identity) is None:
        raise SystemExit(
            "content_only source without .git requires a 40-character lowercase "
            "hex GIT_COMMIT_HASH content identity"
        )
    head = content_identity
else:
    head = (git_dir / "HEAD").read_text(encoding="utf-8").strip()
    if head.startswith("ref: "):
        common_dir = (git_dir / (git_dir / "commondir").read_text().strip()).resolve()
        ref = head[len("ref: "):]
        ref_path = common_dir / ref
        if ref_path.is_file():
            head = ref_path.read_text(encoding="utf-8").strip()
        else:
            suffix = " " + ref
            for line in (common_dir / "packed-refs").read_text(encoding="utf-8").splitlines():
                if line.endswith(suffix):
                    head = line.split(" ", 1)[0]
                    break
            else:
                raise SystemExit(f"cannot resolve worktree ref {ref}")
output.write_text(head + "\n", encoding="utf-8")
PY
  printf '%s\n' \
    'content_only snapshot: source_files.sha256 is authoritative; Git status/diff were not invoked' \
    > "$release/source_status.txt"
  printf '%s\n' \
    'content_only snapshot: no Git-generated worktree diff; see source_files.sha256 and source_change_ledger.md' \
    > "$release/source_worktree.diff"
fi

(
  cd "$repo"
  find fluxon_rs/fluxon_kv fluxon_rs/moka fluxon_rs/fluxon_util -type f \
    ! -path '*/target/*' \
    -print0 \
    | LC_ALL=C sort -z \
    | xargs -0 sha256sum
) > "$release/source_files.sha256"

python3 - "$release" <<'PY'
from pathlib import Path
import hashlib
import sys
import zipfile

release = Path(sys.argv[1])
wheels = sorted(p for p in release.glob("*.whl") if not p.name.startswith("fluxon_pyo3-"))
if len(wheels) != 1:
    raise SystemExit(f"expected one unified Fluxon wheel, got {[p.name for p in wheels]}")
wheel = wheels[0]
with zipfile.ZipFile(wheel) as zf:
    matches = [n for n in zf.namelist() if n.endswith("fluxon_pyo3/fluxon_pyo3.abi3.so")]
    if len(matches) != 1:
        raise SystemExit(f"expected one pyo3 abi3 so in wheel, got {matches}")
    pyo3_bytes = zf.read(matches[0])
sha = hashlib.sha256(pyo3_bytes).hexdigest()
(release / "fluxon_pyo3.abi3.so.sha256").write_text(sha + "\n")
print(f"pyo3_sha256 {sha}")
PY

(
  cd "$release"
  sha256sum \
    memory_pool_host_fluxon_metadata_only.py \
    source_files.sha256 \
    source_head.txt \
    source_status.txt \
    source_worktree.diff \
    source_external_delete_ack_batch.rs \
    source_client_external_api.rs \
    source_client_get.rs \
    source_client_msg_pack.rs \
    source_client_mod.rs \
    source_client_reclaim.rs \
    source_client_put.rs \
    source_master_mod.rs \
    source_master_msg_pack.rs \
    source_master_put.rs \
    source_master_reclaim.rs \
    source_external_client_mod.rs \
    source_fluxon_pyo3_lib.rs \
    source_python_kvclient_interface.py \
    source_python_fluxon.py \
    source_sglang_fluxon_kv_design_cn.md \
    sglang_unified_radix_cache_r38.py \
    sglang_hicache_fluxon_r38.py \
    validate_e44_r38_get_prefix_reuse.py \
    smoke_e44_r38_hicache_adapter.py \
    analyze_e44_r39_get_ready_breakdown.py \
    fluxon_pyo3.abi3.so.sha256 \
    >> fluxon_release.sha256
  sha256sum -c fluxon_release.sha256
)

find "$release" -maxdepth 1 -type f -name '*.whl' -exec sha256sum {} +
