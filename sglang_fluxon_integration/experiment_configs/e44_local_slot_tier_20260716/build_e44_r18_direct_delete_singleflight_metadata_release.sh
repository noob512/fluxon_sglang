...
release=/mnt/nvme0/mjq_build/fluxon_e44_r18_direct_delete_singleflight_metadata_20260719

test "$(findmnt -n -o SOURCE -T "$target")" = /dev/nvme0n1p3
test "$(findmnt -n -o SOURCE -T "$(dirname "$rootfs")")" = /dev/nvme0n1p3
test "$(findmnt -n -o SOURCE -T "$(dirname "$release")")" = /dev/nvme0n1p3
ulimit -n 65535
mkdir -p "$target"
if [ ! -d "$rootfs" ]; then
  sudo rsync -aHAX --numeric-ids /tmp/fluxon_pypack_rootfs/ "$rootfs/"
fi
rm -rf -- "$release"
mkdir -p "$release"
