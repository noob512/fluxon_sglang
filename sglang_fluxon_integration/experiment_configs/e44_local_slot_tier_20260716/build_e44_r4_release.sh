...
release="$repo/fluxon_release_e44_r4_reclaim_remote_get_fence_20260716"
...
# The shared helper uses maxdepth=1 `find -delete`, which cannot remove a
# previously finalized non-empty release directory.  E44 r4 is a generated build
# artifact, so recreate it explicitly before handing it to the helper.
rm -rf -- "$release"
mkdir -p "$release"
