...
def _fluxon_extend_page_hash_chain(
    key: RadixKey,
    page_size: int,
    prior_hash: Optional[str],
) -> list[str]:
    """Hash a page-aligned suffix with the canonical radix hash chain."""
    if page_size <= 0:
        raise ValueError(f"page_size must be positive, got {page_size}")
    if len(key) % page_size != 0:
        raise ValueError(
            "Fluxon suffix hash input must be page aligned: "
            f"tokens={len(key)} page_size={page_size}"
        )

    hashes: list[str] = []
    last_hash = prior_hash
    for start in range(0, len(key), page_size):
        last_hash = key.hash_page(start, start + page_size, last_hash)
        hashes.append(last_hash)
    return hashes


def _fluxon_prefetch_threshold_met(
    recoverable_pages: int,
    page_size: int,
    prefetch_threshold: int,
) -> bool:
    """Apply the transfer threshold to the backend-recoverable prefix."""
    if recoverable_pages < 0 or page_size <= 0 or prefetch_threshold < 0:
        raise ValueError(
            "invalid Fluxon prefetch threshold inputs: "
            f"pages={recoverable_pages} page_size={page_size} "
            f"threshold={prefetch_threshold}"
        )
    return recoverable_pages * page_size >= prefetch_threshold


def _fluxon_build_prefetch_hash_chain(
    radix_hash_values: Sequence[str],
    scheduler_suffix: RadixKey,
    page_size: int,
    canonical_last_hash: Optional[str],
) -> tuple[list[str], list[str]]:
    """Join disjoint radix-lineage keys and the scheduler-provided suffix.

    ``scheduler_suffix`` already starts after ``last_host_node``.  Its tokens
    must therefore be hashed in full from that node's canonical last hash;
    the preceding radix lineage is a key prefix, not an overlapping token
    prefix to remove again.
    """
    synthesized_hash_values = _fluxon_extend_page_hash_chain(
        scheduler_suffix,
        page_size,
        canonical_last_hash,
    )
    return (
        list(radix_hash_values) + synthesized_hash_values,
        synthesized_hash_values,
    )


def _fluxon_storage_match_blocked(
    *,
    node_evicted: bool,
    node_storage_backed: bool,
    matched_tokens: int,
    storage_ceiling_tokens: Optional[int],
) -> bool:
    """Stop only stale storage metadata, never newly computed device nodes."""
    return bool(
        storage_ceiling_tokens is not None
        and matched_tokens >= storage_ceiling_tokens
        and node_evicted
        and node_storage_backed
    )
...
class UnifiedRadixCache(KVCacheEventMixin, BasePrefixCache):
    ...
    def _set_fluxon_hostless_storage_match_ceiling(
        self,
        req: Any,
        restored_node: UnifiedTreeNode,
        *,
        clear_unrestored_hits: bool,
    ) -> int:
        ceiling_tokens = self._node_prefix_len(restored_node)
        previous = getattr(
            req,
            "_fluxon_hostless_storage_match_ceiling_tokens",
            None,
        )
        if previous is not None:
            ceiling_tokens = min(int(previous), ceiling_tokens)
        setattr(
            req,
            "_fluxon_hostless_storage_match_ceiling_tokens",
            ceiling_tokens,
        )
        if clear_unrestored_hits:
            req.host_hit_length = 0
            req.storage_hit_length = 0
            if hasattr(req, "storage_candidate_hit_length"):
                req.storage_candidate_hit_length = 0
        self._observe_fluxon_hostless_request(
            str(getattr(req, "rid", "")),
            storage_match_ceiling_tokens=ceiling_tokens,
        )
        return ceiling_tokens
    ...
    def _install_fluxon_hostless_storage_suffix(
        self,
        parent: UnifiedTreeNode,
        key: RadixKey,
        hash_values: Sequence[str],
    ) -> UnifiedTreeNode:
        """Publish a backend-confirmed suffix into the local radix as metadata.

        The payload remains in Fluxon.  Only pages returned by the common TP
        plan are installed, so ``storage_backed`` never gets inferred from the
        request token stream alone.
        """
        expected_tokens = len(hash_values) * self.page_size
        if len(key) != expected_tokens:
            raise RuntimeError(
                "Fluxon metadata suffix shape mismatch: "
                f"tokens={len(key)} hashes={len(hash_values)} "
        ...
        cursor = parent
        remaining_key = key
        remaining_hashes = list(hash_values)
        while len(remaining_key) > 0:
            child_key = remaining_key.child_key(self.page_size)
            child = cursor.children.get(child_key)
            if child is None:
                break

            prefix_len = child.key.match(remaining_key, page_size=self.page_size)
            if prefix_len <= 0:
                raise RuntimeError(
                    "Fluxon metadata suffix collided with a non-matching radix child: "
                    f"parent={cursor.id} child={child.id}"
                )
            if prefix_len < len(child.key):
                child = self._split_node(child.key, child, prefix_len)

            prefix_pages = prefix_len // self.page_size
            expected_hashes = remaining_hashes[:prefix_pages]
            actual_hashes = self._node_hash_values(child)
            if actual_hashes != expected_hashes:
                raise RuntimeError(
                    "Fluxon metadata suffix hash mismatch: "
                    f"node={child.id} expected_pages={len(expected_hashes)} "
                    f"actual_pages={len(actual_hashes)}"
                )
            child.component_data[BASE_COMPONENT_TYPE].metadata[
                "storage_backed"
            ] = True
            self._update_evictable_leaf_sets(child)
            self._update_evictable_leaf_sets(cursor)
            cursor = child
            remaining_key = remaining_key[prefix_len:]
            remaining_hashes = remaining_hashes[prefix_pages:]

        if len(remaining_key) == 0:
            return cursor

        new_node = UnifiedTreeNode(self.tree_components, priority=cursor.priority)
        new_node.parent = cursor
        new_node.key = remaining_key
        new_node.hash_value = remaining_hashes
        new_node.component_data[BASE_COMPONENT_TYPE].metadata[
            "storage_backed"
        ] = True
        cursor.children[remaining_key.child_key(self.page_size)] = new_node
    ...
    def match_prefix(self, params: MatchPrefixParams) -> MatchResult:
        ...
        storage_ceiling_tokens = None
        if self._is_fluxon_hostless_full_mode() and params.req is not None:
            storage_ceiling_tokens = getattr(
                params.req,
                "_fluxon_hostless_storage_match_ceiling_tokens",
                None,
            )
        ...
        (
            ...
            key,
            storage_ceiling_tokens=storage_ceiling_tokens,
        )
    ...
    def _record_req_lock_handle(
        self,
        req: Any,
        node: Any,
        params: DecLockRefParams,
    ) -> None:
        """Remember the exact node and release replay for a request lock."""
        attr = "_unified_tree_lock_handle"
        if getattr(req, attr, None) is not None:
            raise RuntimeError(
                "request already owns a UnifiedRadix lock handle: "
                f"rid={getattr(req, 'rid', '')}"
            )
        setattr(req, attr, (node, params))

    def _consume_req_lock_handle(
        self,
        req: Any,
    ) -> tuple[Any, DecLockRefParams]:
        """Consume the request lock handle, with a legacy-runtime fallback."""
        attr = "_unified_tree_lock_handle"
        handle = getattr(req, attr, None)
        if handle is not None:
            delattr(req, attr)
            if not isinstance(handle, tuple) or len(handle) != 2:
                raise RuntimeError(
                    "invalid UnifiedRadix request lock handle: "
                    f"rid={getattr(req, 'rid', '')}"
                )
            return handle
        return (
    ...
    def cache_finished_req(self, req: Req, is_insert: bool = True, **kwargs) -> None:
        ...
        lock_node, lock_params = self._consume_req_lock_handle(req)
    ...
    def _match_prefix_helper(
        self,
        key: RadixKey,
        storage_ceiling_tokens: Optional[int] = None,
    ) -> tuple[
        list[torch.Tensor],
        UnifiedTreeNode,
        int,
        ...
        while len(key) > 0 and child_key in node.children:
            ...
            if _fluxon_storage_match_blocked(
                node_evicted=child.evicted,
                node_storage_backed=self._is_component_storage_backed(
                    child.component_data[BASE_COMPONENT_TYPE]
                ),
    ...
    def _split_node(
        ...
        child_full_metadata = child.component_data[BASE_COMPONENT_TYPE].metadata
        if bool(child_full_metadata.get("storage_backed", False)):
            new_node.component_data[BASE_COMPONENT_TYPE].metadata[
                "storage_backed"
            ] = True
    ...
    def prefetch_from_storage(
        ...
        if self._is_fluxon_hostless_full_mode():
            ...
            hash_values: list[str] = []
            radix_hash_values: list[str] = []
            synthesized_hash_values: list[str] = []
            synthesized_key = prefetch_key[:0]
            kv_handle = None
            ...
            radix_hash_values = self._node_hash_values_after_ancestor(
                last_host_node,
                kv_anchor_node,
            )
            ...
            group_nodes = []
            while (
                group_cursor is not kv_anchor_node
                and group_cursor is not self.root_node
            ):
            ...
            canonical_last_hash = last_host_node.get_last_hash_value()
            if last_hash != canonical_last_hash:
                raise RuntimeError(
                    "Fluxon prefetch hash anchor mismatch: "
                    f"req={req_id} node={last_host_node.id} "
                    f"caller={last_hash} canonical={canonical_last_hash}"
                )
            # The scheduler has already sliced at matched_len, so this key is
            # disjoint from (and immediately follows) radix_hash_values.
            synthesized_key = prefetch_key
            (
                hash_values,
                synthesized_hash_values,
            ) = _fluxon_build_prefetch_hash_chain(
                radix_hash_values,
                synthesized_key,
                self.page_size,
                canonical_last_hash,
            )
            # Existing radix nodes remain atomic restore units.  A suffix that
            # is absent from this worker's radix is queried page-by-page, then
            # coalesced into one metadata node for the returned prefix.
            atomic_group_lens.extend([1] * len(synthesized_hash_values))
            requested_pages = len(hash_values)
            self._observe_fluxon_hostless_request(
                req_id,
                prefetch_input_tokens=prefetch_length,
                prefetch_decision=("querying" if hash_values else "no_hash_values"),
                radix_query_pages=len(radix_hash_values),
                synthesized_query_pages=len(synthesized_hash_values),
            ...
            recoverable_tokens = cpu_common_pages * self.page_size
            below_prefetch_threshold = not _fluxon_prefetch_threshold_met(
                cpu_common_pages,
                self.page_size,
                self.prefetch_threshold,
            )
            prefetch_rate_limited = False
            if cpu_common_pages > 0 and not below_prefetch_threshold:
                prefetch_rate_limited = self.cache_controller.prefetch_rate_limited()
            kv_prefetch_enabled = bool(
                cpu_common_pages > 0
                and not below_prefetch_threshold
                and not prefetch_rate_limited
            )
            prefetch_decision = (
                "no_hash_values"
                if not hash_values
            ...
            self._observe_fluxon_hostless_request(
                ...
                recoverable_pages=cpu_common_pages,
                recoverable_tokens=recoverable_tokens,
            )
            if not kv_prefetch_enabled:
                cancel_kv_handle(f"prefetch_{prefetch_decision}")
                cpu_common_pages = 0
                gpu_common_pages = 0
                local_gpu_remote_indices = ()
            ...
            if not hash_values and not mamba_required_for_load_back:
                ...
                return
            restore_host_node = last_host_node
            if hash_values:
                if mamba_required_for_load_back and transferable_pages != requested_pages:
                    ...
                    ready_synthesized_pages = max(
                        0,
                        min(
                            len(synthesized_hash_values),
                            transferable_pages - len(radix_hash_values),
                        ),
                    )
                    if ready_synthesized_pages > 0:
                        try:
                            restore_host_node = (
                                self._install_fluxon_hostless_storage_suffix(
                                    last_host_node,
                                    synthesized_key[
                                        : ready_synthesized_pages * self.page_size
                                    ],
                                    synthesized_hash_values[
                                        :ready_synthesized_pages
                                    ],
                                )
                            )
                        except Exception:
                            cancel_kv_handle("prefetch_metadata_install_error")
                            raise
                    self._observe_fluxon_hostless_request(
                        req_id,
                        installed_synthesized_pages=ready_synthesized_pages,
                        restore_node_id=restore_host_node.id,
                    )
            ...
            operation = _FluxonHostlessPrefetchOperation(
                ...
                anchor_node_id=restore_host_node.id,
                mamba_anchor_node_id=(
                    mamba_state_node.id
                    if mamba_state_node is not None
                    else restore_host_node.id
                ),
        ...
        if self._is_fluxon_hostless_full_mode():
            ...
            if ready_operation is not None and ready_operation.hash_value:
                ...
                if ready_node is None:
                    self._set_fluxon_hostless_storage_match_ceiling(
                        req,
                        last_best_match_device_node,
                        clear_unrestored_hits=True,
                    )
