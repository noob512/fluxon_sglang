...
def _fluxon_hostless_max_atomic_prefix_pages(
    candidate_pages: int,
    atomic_group_lens: Sequence[int],
    page_size: int,
    available_tokens: int,
) -> int:
    """Return the largest whole-group prefix that fits a token budget.

    ``execute_get_plan_*`` accepts a shorter prefix, but only at an atomic
    group boundary.  The old admission path rejected the whole plan when the
    complete prefix did not fit the device-headroom budget.  Keeping this
    calculation side-effect free makes the partial-admission decision easy to
    test and, importantly, prevents a partial radix node from reaching the
    backend.
    """
    candidate_pages = int(candidate_pages)
    page_size = int(page_size)
    available_tokens = int(available_tokens)
    if candidate_pages <= 0 or page_size <= 0 or available_tokens < page_size:
        return 0

    max_pages = available_tokens // page_size
    if max_pages <= 0:
        return 0

    groups = tuple(int(length) for length in atomic_group_lens)
    if not groups:
        groups = (1,) * candidate_pages
    if any(length <= 0 for length in groups):
        raise ValueError(
            "Fluxon hostless atomic group lengths must be positive: "
            f"groups={groups}"
        )

    selected_pages = 0
    for group_len in groups:
        next_pages = selected_pages + group_len
        if next_pages > candidate_pages or next_pages > max_pages:
            break
...
class UnifiedRadixCache(KVCacheEventMixin, BasePrefixCache):
    ...
    def _reset_full(self) -> None:
        ...
        self._fluxon_hostless_admission_partial = 0
        self._fluxon_hostless_admission_partial_tokens = 0
    ...
    def _try_acquire_fluxon_hostless_prefetch_admission(
        ...
        return {
            ...
            "device_headroom_available_tokens": max(
                0,
                device_prefetch_budget - total_before,
            ),
        }
        ...
        **fields: Any,
        ...
        if observation is None:
            observation = {
                ...
                "lineage_key_ids": (),
                "lineage_sources": "",
                "source_admission_reason": "not_observed",
                "source_admission_initial_pages": 0,
                "source_admission_final_pages": 0,
                "source_admission_partial": 0,
                "source_admission_initial_tokens": 0,
                "source_admission_partial_tokens": 0,
            }
    ...
    def _finish_fluxon_hostless_request_observation(
        ...
        **fields: Any,
        ...
        if int(observation.get("source_admission_partial", 0)):
            counters["source_admission_partial"] += 1
            counters["source_admission_partial_tokens"] += int(
                observation.get("source_admission_partial_tokens", 0)
            )
        ...
        logger.info(
            "Fluxon hostless source admission result: req=%s terminal=%s "
            "reason=%s partial=%d initial_pages=%d final_pages=%d "
            "initial_tokens=%d partial_tokens=%d",
            req_id,
            terminal,
            str(observation.get("source_admission_reason", "not_observed")),
            int(observation.get("source_admission_partial", 0)),
            int(observation.get("source_admission_initial_pages", 0)),
            int(observation.get("source_admission_final_pages", 0)),
            int(observation.get("source_admission_initial_tokens", 0)),
            int(observation.get("source_admission_partial_tokens", 0)),
        )
        logger.info(
    ...
    def _log_fluxon_hostless_admission_snapshot(self, caller: str) -> None:
        logger.info(
            ...
            "device_headroom_rejected=%d partial=%d partial_tokens=%d "
            "source_mismatches=%d "
            ...
            int(getattr(self, "_fluxon_hostless_admission_partial", 0)),
            int(getattr(self, "_fluxon_hostless_admission_partial_tokens", 0)),
            int(
                getattr(
                    self,
    ...
    def prefetch_from_storage(
        ...
        if self._is_fluxon_hostless_full_mode():
            ...
            source_admission_initial_pages = 0
            source_admission_initial_tokens = 0
            source_admission_partial = False
            source_admission_partial_tokens = 0
            ...
            if kv_handle is not None and transferable_pages > 0:
                source_admission_initial_pages = int(transferable_pages)
                source_admission_initial_tokens = (
                    source_admission_initial_pages * self.page_size
                )
                ...
                # A complete plan can be larger than the currently available
                # device headroom even though one or more whole radix nodes
                # still fit.  Preserve the 8,192-token safety margin, but
                # consume the largest atomic-group prefix that fits instead of
                # throwing away the entire remote plan.  The second admission
                # call rechecks all limits against the live state; no debt is
                # charged for the rejected full candidate.
                if (
                    not bool(admission["admitted"])
                    and str(admission["reason"]) == "device_headroom"
                    and not mamba_required_for_load_back
                ):
                    available_headroom_tokens = int(
                        admission["device_headroom_available_tokens"]
                    )
                    partial_pages = _fluxon_hostless_max_atomic_prefix_pages(
                        transferable_pages,
                        source_group_lens,
                        self.page_size,
                        available_headroom_tokens,
                    )
                    if self.tp_world_size > 1:
                        ...
                        partial_state_min = partial_state.clone()
                        partial_state_max = partial_state.clone()
                        torch.distributed.all_reduce(
                            partial_state_min,
                            op=torch.distributed.ReduceOp.MIN,
                            group=self.tp_group,
                        ...
                        if int(partial_state_min.item()) != int(
                            partial_state_max.item()
                        ):
                            partial_pages = 0
                        else:
                            partial_pages = int(partial_state_min.item())
                    if 0 < partial_pages < transferable_pages:
                        partial_remote_pages = sum(
                            1
                            for index in local_gpu_remote_indices
                            if index < partial_pages
                        )
                        partial_admission = (
                            self._try_acquire_fluxon_hostless_prefetch_admission(
                                req_id,
                                partial_pages * self.page_size,
                                partial_remote_pages,
                            )
                        )
                        if bool(partial_admission["admitted"]):
                            source_admission_partial = True
                            source_admission_partial_tokens = (
                                partial_pages * self.page_size
                            )
                            self._fluxon_hostless_admission_partial += 1
                            self._fluxon_hostless_admission_partial_tokens += (
                                source_admission_partial_tokens
                            )
                            logger.info(
                                "Fluxon hostless source admission partial: "
                                "req=%s full_pages=%d admitted_pages=%d "
                                "full_tokens=%d admitted_tokens=%d "
                                "available_tokens=%d groups=%s",
                                req_id,
                                source_admission_initial_pages,
                                partial_pages,
                                source_admission_initial_tokens,
                                source_admission_partial_tokens,
                                available_headroom_tokens,
                            ...
                            transferable_pages = partial_pages
                            local_selected_remote_pages = partial_remote_pages
                ...
                self._observe_fluxon_hostless_request(
                    ...
                    source_admission_initial_pages=source_admission_initial_pages,
                    source_admission_final_pages=(
                        transferable_pages if bool(admission["admitted"]) else 0
                    ),
                    source_admission_partial=int(source_admission_partial),
                    source_admission_initial_tokens=source_admission_initial_tokens,
                    source_admission_partial_tokens=source_admission_partial_tokens,
                )
                if not admission_active:
