...
class _FluxonHostlessPrefetchOperation:
    def __init__(
        ...
        atomic_group_lens: tuple[int, ...] = (),
        host_prefetch_keys: tuple[str, ...] = (),
        host_prefetch_target_pages: int = 0,
        deferred_materialization: bool = False,
        gdr_budget_pages: int = 0,
    ) -> None:
        self.backend = backend
        ...
        self.atomic_group_lens = tuple(int(length) for length in atomic_group_lens)
        self.host_prefetch_keys = tuple(str(key) for key in host_prefetch_keys)
        self.host_prefetch_target_pages = max(0, int(host_prefetch_target_pages))
        self.host_prefetch_started = bool(self.host_prefetch_keys)
        self.host_prefetch_start_position = -1
        self.deferred_materialization = bool(deferred_materialization)
        self.gdr_budget_pages = max(0, int(gdr_budget_pages))
        self.materialize_started = False
        self.host_prefetch_stats: dict[str, Any] = {}
        self.mamba_plan_ptr = mamba_plan_ptr
        ...
        self._finished = not self.deferred_materialization
...
class UnifiedRadixCache(KVCacheEventMixin, BasePrefixCache):
    def __init__(
        ...
        self._fluxon_two_stage_materialization_enabled = False
        self._fluxon_host_prefetch_queue_head_k = 6
        self._fluxon_host_prefetch_budget_pages = 96
        self._fluxon_host_correction_enabled = False
        self._fluxon_materialize_queue_head_k = 3
        self._fluxon_materialize_gdr_budget_pages = 96
        self._fluxon_hostless_plan_cache = None
        self._fluxon_hostless_mamba_plan_cache = None
        layer_batch_dma_raw = os.getenv(
            "SGLANG_FLUXON_HOSTLESS_LAYER_BATCH_DMA", "0"
        )
        if layer_batch_dma_raw not in ("0", "1"):
            raise ValueError(
                "SGLANG_FLUXON_HOSTLESS_LAYER_BATCH_DMA must be 0 or 1, "
                f"got {layer_batch_dma_raw!r}"
            )
        self._fluxon_hostless_layer_batch_dma_enabled = (
            layer_batch_dma_raw == "1"
        )
        background_dma_raw = os.getenv(
            "SGLANG_FLUXON_HOSTLESS_BACKGROUND_DMA_SUBMIT", "0"
        )
        if background_dma_raw not in ("0", "1"):
            raise ValueError(
                "SGLANG_FLUXON_HOSTLESS_BACKGROUND_DMA_SUBMIT must be 0 or 1, "
                f"got {background_dma_raw!r}"
            )
        self._fluxon_hostless_background_dma_submit_enabled = (
            background_dma_raw == "1"
        )
        if (
            self._fluxon_hostless_background_dma_submit_enabled
            and not self._fluxon_hostless_layer_batch_dma_enabled
        ):
            raise ValueError(
                "Fluxon background DMA submit requires "
                "SGLANG_FLUXON_HOSTLESS_LAYER_BATCH_DMA=1"
            )
        self._fluxon_hostless_dma_submit_executor = (
            ThreadPoolExecutor(
                max_workers=1,
                thread_name_prefix="fluxon-h2d-submit",
            )
            if self._fluxon_hostless_background_dma_submit_enabled
            else None
        )
        ...
        if self._fluxon_hostless_dma_submit_executor is not None:
            # Resolve an index-less `cuda` device on the scheduler thread.
            # A fresh Python worker thread otherwise starts with CUDA device 0,
            # which makes TP1 create its submit stream on the wrong GPU.
            self._fluxon_hostless_cuda_device_id = self._cuda_device_index()
            # Establish the CUDA device/context and create the submit stream on
            # the worker itself before serving requests. In particular, do not
            # reuse HybridCacheController.load_stream across Python threads.
            self._fluxon_hostless_dma_submit_executor.submit(
                self._fluxon_hostless_background_dma_stream
            ).result()
        self._fluxon_hostless_layer_batch_dma_batches = 0
        self._fluxon_hostless_layer_batch_dma_pages = 0
        validate_every_raw = os.getenv(
            "SGLANG_FLUXON_HOSTLESS_PAGE_INDEX_VALIDATE_EVERY_N", "0"
        )
        try:
            validate_every_n = int(validate_every_raw)
        except ValueError as err:
            raise ValueError(
                "SGLANG_FLUXON_HOSTLESS_PAGE_INDEX_VALIDATE_EVERY_N must be "
                f"a non-negative integer, got {validate_every_raw!r}"
            ) from err
        if validate_every_n < 0:
            raise ValueError(
                "SGLANG_FLUXON_HOSTLESS_PAGE_INDEX_VALIDATE_EVERY_N must be "
                f"non-negative, got {validate_every_n}"
            )
        self._fluxon_hostless_page_index_validate_every_n = validate_every_n
        self._fluxon_hostless_page_index_call_count = 0
        if validate_every_n:
            logger.warning(
                "Fluxon hostless page-index CUDA validation enabled every %d calls; "
                "validation synchronizes the active CUDA stream",
                validate_every_n,
            )
        eviction_write_stream_raw = os.getenv(
            "SGLANG_FLUXON_HOSTLESS_EVICTION_WRITE_STREAM", "0"
        )
        if eviction_write_stream_raw not in ("0", "1"):
            raise ValueError(
                "SGLANG_FLUXON_HOSTLESS_EVICTION_WRITE_STREAM must be 0 or 1, "
                f"got {eviction_write_stream_raw!r}"
            )
        self._fluxon_hostless_eviction_write_stream_enabled = (
            eviction_write_stream_raw == "1"
        )
        if self._fluxon_hostless_eviction_write_stream_enabled:
            logger.warning(
                "Fluxon hostless eviction writes use the HiCache write stream "
                "after a per-node source-ready event"
            )
    ...
    def _reset_full(self) -> None:
        ...
        self._empty_match_result = MatchResult(
            ...
            mamba_state_node=self.root_node,
        )
    ...
    def init_hicache(self, server_args: ServerArgs, params: CacheInitParams) -> None:
        ...
        if self._fluxon_hostless_layer_batch_dma_enabled:
            if str(storage_backend).lower() != "fluxon":
                raise ValueError(
                    "Fluxon layer-batched H2D DMA requires "
                    "hicache_storage_backend=fluxon"
                )
            logger.warning(
        ...
        if storage_backend is not None:
            ...
            if str(storage_backend).lower() == "fluxon":
                ...
                two_stage_raw = storage_extra_config.get(
                    "two_stage_mixed_materialization", False
                )
                if isinstance(two_stage_raw, bool):
                    two_stage_enabled = two_stage_raw
                elif isinstance(two_stage_raw, (int, float)):
                    two_stage_enabled = bool(two_stage_raw)
                else:
                    normalized = str(two_stage_raw).strip().lower()
                    if normalized not in ("0", "1", "false", "true"):
                        raise ValueError(
                            "two_stage_mixed_materialization must be boolean, "
                            f"got {two_stage_raw!r}"
                        )
                    two_stage_enabled = normalized in ("1", "true")
                queue_head_k = int(
                    storage_extra_config.get("materialize_queue_head_k", 3)
                )
                host_prefetch_queue_head_k = int(
                    storage_extra_config.get(
                        "host_prefetch_queue_head_k",
                        max(6, queue_head_k + 1),
                    )
                )
                host_prefetch_budget_pages = int(
                    storage_extra_config.get("host_prefetch_budget_pages", 96)
                )
                host_correction_raw = storage_extra_config.get(
                    "host_correction_enabled", False
                )
                if isinstance(host_correction_raw, bool):
                    host_correction_enabled = host_correction_raw
                elif isinstance(host_correction_raw, (int, float)):
                    host_correction_enabled = bool(host_correction_raw)
                else:
                    normalized = str(host_correction_raw).strip().lower()
                    if normalized not in ("0", "1", "false", "true"):
                        raise ValueError(
                            "host_correction_enabled must be boolean, "
                            f"got {host_correction_raw!r}"
                        )
                    host_correction_enabled = normalized in ("1", "true")
                gdr_budget_pages = int(
                    storage_extra_config.get("materialize_gdr_budget_pages", 96)
                )
                if queue_head_k <= 0:
                    raise ValueError(
                        "materialize_queue_head_k must be positive, "
                        f"got {queue_head_k}"
                    )
                if host_prefetch_queue_head_k <= queue_head_k:
                    raise ValueError(
                        "host_prefetch_queue_head_k must be greater than "
                        "materialize_queue_head_k so the host stage starts "
                        "earlier: "
                        f"host={host_prefetch_queue_head_k} "
                        f"materialize={queue_head_k}"
                    )
                if not 0 <= host_prefetch_budget_pages <= _FLUXON_GPU_DIRECT_STAGING_SLOT_COUNT:
                    raise ValueError(
                        "host_prefetch_budget_pages must be within the GPU "
                        "staging capacity: "
                        f"budget={host_prefetch_budget_pages} "
                        f"capacity={_FLUXON_GPU_DIRECT_STAGING_SLOT_COUNT}"
                    )
                if not 0 < gdr_budget_pages <= _FLUXON_GPU_DIRECT_STAGING_SLOT_COUNT:
                    raise ValueError(
                        "materialize_gdr_budget_pages must be within the GPU "
                        "staging capacity: "
                        f"budget={gdr_budget_pages} "
                        f"capacity={_FLUXON_GPU_DIRECT_STAGING_SLOT_COUNT}"
                    )
                self._fluxon_two_stage_materialization_enabled = two_stage_enabled
                self._fluxon_host_prefetch_queue_head_k = (
                    host_prefetch_queue_head_k
                )
                self._fluxon_host_prefetch_budget_pages = (
                    host_prefetch_budget_pages
                )
                self._fluxon_host_correction_enabled = host_correction_enabled
                self._fluxon_materialize_queue_head_k = queue_head_k
                self._fluxon_materialize_gdr_budget_pages = gdr_budget_pages
                if two_stage_enabled:
                    logger.warning(
                        "Fluxon two-stage mixed materialization enabled: "
                        "host_queue_head_k=%d materialize_queue_head_k=%d "
                        "host_budget_pages=%d host_correction_enabled=%s "
                        "gdr_budget_pages=%d staging_slots=%d",
                        host_prefetch_queue_head_k,
                        queue_head_k,
                        host_prefetch_budget_pages,
    ...
    def _finish_fluxon_hostless_request_observation(
        ...
        **fields: Any,
        ...
        if "materialize_gdr_budget_pages" in observation:
            counters["materialize_host_ready_pages"] += int(
                observation.get("host_prefetch_ready_pages", 0)
            )
            counters["materialize_h2d_source_pages"] += int(
                observation.get("materialize_h2d_source_pages", 0)
            )
            counters["materialize_gdr_source_pages"] += int(
                observation.get("materialize_gdr_source_pages", 0)
            )
            logger.info(
                "Fluxon mixed materialization lifecycle: req=%s tp_rank=%d "
                "terminal=%s decision=%s host_queue_head_k=%d host_start_pos=%d "
                "materialize_queue_head_k=%d materialize_pos=%d "
                "gdr_budget_pages=%d host_target_pages=%d host_tracked_pages=%d "
                "host_ready_pages=%d host_waited_pages=%d "
                "host_terminal_drained_pages=%d host_failed_pages=%d "
                "host_wait_ms=%.3f correction_requested_pages=%d "
                "correction_ready_pages=%d h2d_source_pages=%d "
                "gdr_source_pages=%d materialize_duration_ms=%.3f",
                req_id,
                self._fluxon_hostless_tp_rank(),
                terminal,
                decision,
                int(observation.get("host_prefetch_queue_head_k", 0)),
                int(observation.get("host_prefetch_start_queue_position", -1)),
                int(observation.get("materialize_queue_head_k", 0)),
                int(observation.get("materialize_queue_position", -1)),
                int(observation.get("materialize_gdr_budget_pages", 0)),
                int(observation.get("host_prefetch_target_pages", 0)),
                int(observation.get("host_prefetch_tracked_pages", 0)),
                int(observation.get("host_prefetch_ready_pages", 0)),
                int(observation.get("host_prefetch_waited_pages", 0)),
                int(observation.get("host_prefetch_terminal_drained_pages", 0)),
                int(observation.get("host_prefetch_failed_pages", 0)),
                float(observation.get("host_prefetch_wait_ms", 0.0)),
                int(observation.get("host_correction_requested_pages", 0)),
                int(observation.get("host_correction_ready_pages", 0)),
                int(observation.get("materialize_h2d_source_pages", 0)),
                int(observation.get("materialize_gdr_source_pages", 0)),
                float(observation.get("materialize_duration_ms", 0.0)),
            )
        logger.info(
    ...
    def _log_fluxon_hostless_observation_snapshot(self, caller: str) -> None:
        counters = self._fluxon_hostless_observation_counters
        if not counters and not self._fluxon_hostless_request_observations:
            return
        summary = ",".join(
    ...
    def _fluxon_hostless_ready_operation_covers_kv(
        ...
        if operation.kv_plan_ptr is None:
            return False
        if exact and len(operation.hash_value) != len(hash_values):
            return False
        if operation.completed_tokens < len(hash_values) * self.page_size:
            return False
        return operation.hash_value[: len(hash_values)] == hash_values
    ...
    def _fluxon_hostless_longest_ready_restore_node(
        ...
        for node in reversed(reverse_path):
            ...
            if next_pages > ready_pages:
                break
            if operation.hash_value[consumed_pages:next_pages] != node_hashes:
                break
    ...
    def _match_post_processor(
        ...
        result = MatchResult(
            ...
            host_hit_length=0,
            mamba_state_node=mamba_state_node,
        )
        ...
        if self._is_fluxon_hostless_full_mode():
            ...
            try:
                ...
                if kv_tokens > 0:
                    ...
                    if self._fluxon_hostless_ready_operation_covers_kv(
                        ...
                        if ready_prefetch_operation.gpu_direct:
                            ...
                            remote_pages = sum(
                                index < len(kv_batch_hash_values)
                ...
                if kv_tokens > 0:
                    ...
                    try:
                        ...
                        if layerwise_async:
                            ...
                            ordered_value_ptrs = backend.view_value_ptrs(
                                kv_plan_ptr,
                ...
                if layerwise_async:
                    ...
                    value_ptrs = backend.view_value_ptrs(
                        kv_plan_ptr,
    ...
    def prefetch_from_storage(
        ...
        if self._is_fluxon_hostless_full_mode():
            ...
            prefetch_rate_limited = False
            if not below_prefetch_threshold:
                prefetch_rate_limited = self.cache_controller.prefetch_rate_limited()
            kv_prefetch_enabled = (
                not below_prefetch_threshold and not prefetch_rate_limited
            )
            self._observe_fluxon_hostless_request(
                req_id,
            ...
            if (
                self._fluxon_two_stage_materialization_enabled
                and hash_values
                and not mamba_required_for_load_back
            ):
                if not hasattr(backend, "prefetch_to_host") or not hasattr(
                    backend, "finish_prefetch_to_host"
                ):
                    raise RuntimeError(
                        "Fluxon two-stage materialization requires "
                        "prefetch_to_host/finish_prefetch_to_host backend APIs"
                    )
                host_prefetch_target = max(
                    0,
                    min(
                        self._fluxon_host_prefetch_budget_pages,
                        min(
                        ...
                        - self._fluxon_materialize_gdr_budget_pages,
                    )
                )
                total_tokens = requested_pages * self.page_size
                operation = _FluxonHostlessPrefetchOperation(
                    backend=backend,
                    ...
                    anchor_node_id=last_host_node.id,
                    has_ready_transfer=False,
                    atomic_group_lens=tuple(atomic_group_lens),
                ...
                self._observe_fluxon_hostless_request(
                    req_id,
                    prefetch_decision="host_prefetch_deferred",
                    requested_pages=requested_pages,
                    host_prefetch_target_pages=host_prefetch_target,
                    host_prefetch_tracked_pages=0,
                    host_prefetch_queue_head_k=(
                        self._fluxon_host_prefetch_queue_head_k
                    ),
                    host_prefetch_budget_pages=(
                    ...
                    host_correction_enabled=self._fluxon_host_correction_enabled,
                    materialize_gdr_budget_pages=operation.gdr_budget_pages,
                )
                logger.info(
                    "HiCache prefetch submitted: req=%s tokens=%d "
                    "mode=fluxon_two_stage_deferred pages=%d host_target=%d "
                    "host_queue_head_k=%d materialize_queue_head_k=%d "
                    "gdr_budget=%d occupied=%d",
                    req_id,
                    total_tokens,
                    requested_pages,
                    host_prefetch_target,
                    self._fluxon_host_prefetch_queue_head_k,
                    self._fluxon_materialize_queue_head_k,
                    operation.gdr_budget_pages,
            ...
            if gpu_reservation_tp_inconsistent:
            ...
            operation = _FluxonHostlessPrefetchOperation(
                ...
                mamba_anchor_node_id=(
                    mamba_state_node.id if mamba_state_node is not None else last_host_node.id
                ),
            ...
            logger.info(
                ...
                "gpu_direct=%s mamba=%s occupied=%d",
                req_id,
                total_tokens,
                requested_pages,
                transferable_pages,
    ...
    def _start_fluxon_host_prefetch(
        self,
        req_id: str,
        operation: _FluxonHostlessPrefetchOperation,
        queue_position: int,
    ) -> None:
        if operation.host_prefetch_started or operation.is_terminated():
            return
        host_prefetch_keys = operation.backend.prefetch_to_host(
            operation.hash_value,
            component_name=PoolName.KV,
            max_keys=operation.host_prefetch_target_pages,
            release_on_complete=True,
        )
        operation.host_prefetch_keys = tuple(host_prefetch_keys)
        operation.host_prefetch_started = True
        operation.host_prefetch_start_position = int(queue_position)
        self._observe_fluxon_hostless_request(
            req_id,
            prefetch_decision="host_prefetch_submitted",
            host_prefetch_target_pages=operation.host_prefetch_target_pages,
            host_prefetch_tracked_pages=len(host_prefetch_keys),
            host_prefetch_start_queue_position=int(queue_position),
            host_prefetch_start_age_ms=(
                self._fluxon_hostless_observation_age_ms(req_id)
            ),
        )
        logger.info(
            "Fluxon bounded host prefetch started: req=%s queue_position=%d "
            "target_pages=%d tracked_pages=%d host_queue_head_k=%d",
            req_id,
            queue_position,
            operation.host_prefetch_target_pages,
            len(host_prefetch_keys),
            self._fluxon_host_prefetch_queue_head_k,
        )

    def _materialize_fluxon_hostless_prefetch(
        self,
        req_id: str,
        operation: _FluxonHostlessPrefetchOperation,
    ) -> None:
        """Turn an early host warmup into one late ordered H2D/GDR plan."""
        if not operation.deferred_materialization or operation._finished:
            return
        if operation.materialize_started:
            raise RuntimeError(
                f"Fluxon materialization was re-entered for request {req_id}"
            )
        operation.materialize_started = True
        materialize_start = time.perf_counter()
        backend = operation.backend
        kv_handle = None
        kv_handle_mode = "none"
        gpu_staging_lease = None
        warm_keepalives: list[Any] = []
        correction_keepalives: list[Any] = []

        def cancel_live_handle(caller: str) -> None:
            nonlocal kv_handle, kv_handle_mode, gpu_staging_lease
            self._cancel_fluxon_hostless_get_start_handle(
                backend,
                kv_handle,
                caller,
                gpu_direct=kv_handle_mode == "gpu",
                plan_only=kv_handle_mode == "plan",
            )
            kv_handle = None
            kv_handle_mode = "none"
            if gpu_staging_lease is not None:
                lease = gpu_staging_lease
                gpu_staging_lease = None
                self._release_fluxon_gpu_staging_lease(lease, caller)
        ...
        try:
            if operation.is_terminated():
                ...
                self._observe_fluxon_hostless_request(
                    req_id,
                    prefetch_decision="terminated_before_materialize",
                )
                return

            warm_keepalives, host_stats = backend.finish_prefetch_to_host(
                operation.host_prefetch_keys,
                component_name=PoolName.KV,
            )
            operation.host_prefetch_stats = dict(host_stats)
            self._observe_fluxon_hostless_request(
                req_id,
                host_prefetch_ready_pages=int(host_stats.get("ready", 0)),
                host_prefetch_waited_pages=int(host_stats.get("waited", 0)),
                host_prefetch_failed_pages=int(host_stats.get("failed", 0)),
                host_prefetch_missing_pages=int(host_stats.get("missing", 0)),
                host_prefetch_terminal_drained_pages=int(
                    host_stats.get("terminal_drained", 0)
                ),
                host_prefetch_wait_ms=float(host_stats.get("wait_ms", 0.0)),
                materialize_start_age_ms=(
                    self._fluxon_hostless_observation_age_ms(req_id)
                ),
            )
            ...
            kv_handle = backend.get_plan(
                operation.hash_value,
                prefix_best_effort=True,
                atomic_group_lens=list(operation.atomic_group_lens),
            )
            kv_handle_mode = "plan"

            # If the early bounded warmup selected owner-local pages instead of
            # remote ones, use the metadata-only plan to identify the excess
            # remote positions.  Revoke that plan before warming the excess,
            # so no key can have a host transfer and a GDR transfer in flight
            # at the same time.  One final plan then observes the new locality.
            initial_gpu_pages = int(
                getattr(kv_handle.gpu_result, "transferable_len", 0)
            )
            initial_remote_indices = tuple(
                int(index)
                for index in getattr(kv_handle, "gpu_remote_indices", ())
                if int(index) < initial_gpu_pages
            )
            correction_keys: list[str] = []
            if (
                self._fluxon_host_correction_enabled
                and len(initial_remote_indices) > operation.gdr_budget_pages
            ):
                correction_indices = initial_remote_indices[
                    operation.gdr_budget_pages :
                ]
                correction_keys = [
                    operation.hash_value[index] for index in correction_indices
                ]
                backend.cancel_get_plan(kv_handle)
                kv_handle = None
                kv_handle_mode = "none"
                correction_tracked = backend.prefetch_to_host(
                    correction_keys,
                    component_name=PoolName.KV,
                    max_keys=len(correction_keys),
                    release_on_complete=False,
                )
                correction_keepalives, correction_stats = (
                    backend.finish_prefetch_to_host(
                        correction_tracked,
                        component_name=PoolName.KV,
                    )
                ...
                self._observe_fluxon_hostless_request(
                    req_id,
                    host_correction_requested_pages=len(correction_keys),
                    host_correction_tracked_pages=len(correction_tracked),
                    host_correction_ready_pages=int(
                        correction_stats.get("ready", 0)
                    ),
                    host_correction_wait_ms=float(
                        correction_stats.get("wait_ms", 0.0)
                    ),
                )
                kv_handle = backend.get_plan(
                    operation.hash_value,
            ...
            self._observe_fluxon_hostless_request(
                req_id,
                plan_ready_age_ms=self._fluxon_hostless_observation_age_ms(req_id),
            )

            plan_state_min = torch.tensor(
                [local_cpu_plan_pages, local_gpu_plan_pages],
                dtype=torch.int,
            )
            plan_state_max = plan_state_min.clone()
            if self.tp_world_size > 1:
            ...
            source_group_lens = list(operation.atomic_group_lens) or [
                1
            ...
            for label, common_pages in (
                ...
                if group_cursor_pages != common_pages:
                    cancel_live_handle(
                        f"materialize_{label}_common_prefix_split_atomic_group"
                    )
                    raise RuntimeError(
                        "Fluxon TP common materialization prefix split an "
                        f"atomic group: req={req_id} mode={label} "
                        f"common={common_pages} group_pages={group_cursor_pages} "
                        f"groups={source_group_lens}"
                    )
            ...
            local_gpu_remote_pages = len(gpu_common_remote_indices)
            gpu_admission_block_reason = None
            if gpu_common_pages <= 0:
                gpu_admission_block_reason = "no_gpu_transferable_prefix"
            elif gpu_common_pages != cpu_common_pages:
                gpu_admission_block_reason = "gpu_prefix_shorter_than_cpu"
            elif local_gpu_remote_pages <= 0:
                gpu_admission_block_reason = "no_remote_sources"
            elif local_gpu_remote_pages > operation.gdr_budget_pages:
                gpu_admission_block_reason = "remote_exceeds_gdr_budget"

            gpu_staging_lease, gpu_staging_admission = (
                backend.try_reserve_gpu_direct_staging(
                    local_gpu_remote_pages,
                    admission_block_reason=gpu_admission_block_reason,
                )
            ...
            if not gpu_reservation_consistent and gpu_staging_lease is not None:
                ...
                self._release_fluxon_gpu_staging_lease(
                    lease,
                    "materialize_gpu_reservation_fallback",
                )
            ...
            if transferable_pages > 0:
                ...
                cancel_live_handle("materialize_zero_common_plan")

            gpu_direct_admission_reason = str(
                gpu_staging_admission["reason"]
            )
            if gpu_reservation_tp_inconsistent:
                gpu_direct_admission_reason = "tp_reservation_inconsistent"
            h2d_source_pages = (
                transferable_pages - len(selected_gpu_remote_indices)
                if kv_gpu_direct
                else transferable_pages
            )
            self._observe_fluxon_hostless_request(
                req_id,
                final_transferable_pages=transferable_pages,
                ...
                gpu_direct_tp_min_pages=int(gpu_reservation_min[1].item()),
                gpu_direct_tp_max_pages=int(gpu_reservation_max[1].item()),
                materialize_h2d_source_pages=h2d_source_pages,
                materialize_gdr_source_pages=len(selected_gpu_remote_indices),
            ...
            operation.hash_value = operation.hash_value[:transferable_pages]
            operation.completed_tokens = transferable_pages * self.page_size
            operation.has_ready_transfer = transferable_pages > 0
            operation.kv_handle = kv_handle
            kv_handle = None
            operation.gpu_staging_lease = gpu_staging_lease
            gpu_staging_lease = None
            operation.gpu_direct = kv_gpu_direct
            operation.gpu_remote_indices = selected_gpu_remote_indices
            operation.deferred_materialization = False
            previous_decision = self._observe_fluxon_hostless_request(req_id).get(
                "prefetch_decision"
            )
            self._observe_fluxon_hostless_request(
                ...
                prefetch_decision=(
                    "materialized_mixed"
                    if kv_gpu_direct and h2d_source_pages > 0
                    else (
                        "materialized_gdr"
                        if kv_gpu_direct
                        else "materialized_h2d"
                    )
                ),
                host_stage_decision=previous_decision,
            )
            logger.info(
                "Fluxon two-stage materialized: req=%s pages=%d h2d_pages=%d "
                "gdr_pages=%d gpu_direct=%s host_ready=%d correction=%d "
                "duration_ms=%.3f",
                req_id,
                transferable_pages,
                h2d_source_pages,
                len(selected_gpu_remote_indices),
                kv_gpu_direct,
                len(warm_keepalives),
                len(correction_keys),
            ...
            cancel_live_handle("materialize_error")
            operation.hash_value = []
            operation.completed_tokens = 0
            operation.has_ready_transfer = False
            operation.gpu_direct = False
            operation.gpu_remote_indices = ()
            operation.deferred_materialization = False
            self._observe_fluxon_hostless_request(
                req_id,
                prefetch_decision="materialize_error",
                materialize_error_type=type(exc).__name__,
                materialize_duration_ms=(
                    time.perf_counter() - materialize_start
                )
                * 1000.0,
            ...
            logger.warning(
                "Fluxon two-stage materialization failed; treating as cache "
                "miss: req=%s error=%s",
                req_id,
                exc,
            )
        finally:
            warm_keepalives.clear()
            correction_keepalives.clear()
    ...
    def check_prefetch_progress(
        ...
        if isinstance(operation, _FluxonHostlessPrefetchOperation):
            if operation.deferred_materialization and not operation.is_finished():
                position = (
                    0
                    if queue_position is None
                    else max(0, int(queue_position))
                )
                if (
                    not operation.host_prefetch_started
                    and position < self._fluxon_host_prefetch_queue_head_k
                ):
                    self._start_fluxon_host_prefetch(
                        req_id,
                        operation,
                        position,
                    )
                if position >= self._fluxon_materialize_queue_head_k:
                    self._observe_fluxon_hostless_request(
                        req_id,
                        host_prefetch_deferred_queue_position=(
                            position
                            if not operation.host_prefetch_started
                            else -1
                        ),
                        host_prefetch_queue_head_k=(
                            self._fluxon_host_prefetch_queue_head_k
                        ),
                        materialize_deferred_queue_position=position,
                ...
                if not operation.host_prefetch_started:
                    raise RuntimeError(
                        "Fluxon host prefetch was not started before late "
                        f"materialization: req={req_id} position={position}"
                    )
                self._observe_fluxon_hostless_request(
                    req_id,
                    materialize_queue_position=position,
                    materialize_queue_head_k=self._fluxon_materialize_queue_head_k,
                )
                self._materialize_fluxon_hostless_prefetch(req_id, operation)
            ...
            self.cache_controller.prefetch_tokens_occupied -= operation.total_tokens
        ...
        def _drain_revoke():
            ...
            for req_id in _drain_queue(cc.prefetch_revoke_queue, n_revoke):
                ...
                if isinstance(operation, _FluxonHostlessPrefetchOperation):
                    ...
                    cc.prefetch_tokens_occupied -= operation.total_tokens
