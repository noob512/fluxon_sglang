...
variant=tier1_independent_005_netobs_enddepth288_get_prefix_reuse
...
tag="fluxon_${E44_PERF_RUN_ID}_adapter_smoke_s96_t2_sys8192_out8_c24_20260720"
namespace="agent_hit50_long_s96_t2_sys8192_v2_${E44_PERF_RUN_ID}_adapter_smoke"
...
(
  ...
  "$python" workload_agent_multiturn_long_context.py --cluster-state "$script_dir/cluster_e44_r11.env" \
    ...
    --run-tag "$tag" --output-root "$workdir/results" --prefix-namespace "$namespace" --request-timeout-s 300 --max-runtime-s 900 \
    --phase-mode router --router-base-url http://10.233.114.129:32000 --base-node node0 --measure-node node1 --seed-node node0 \
    --sessions 96 --turns 2 --agent-groups 96 --system-tokens 8192 --user-tokens 32 --tool-result-tokens 64 --assistant-history-tokens 32 \
