...
local_release=/mnt/nvme0/mjq_build/fluxon_e44_r38_get_prefix_reuse_20260720
remote_release=/storage/mjq/sglang_fluxon/releases/fluxon_e44_r38_get_prefix_reuse_20260720
...
expected_wheel_sha256=66566ba1f0034205fb53fce35f3684080520103bb0832cd9d271939c2a1d0b1c
expected_pyo3_sha256=3e5b9d41af89357d57f09664a4029ef5c12b189b32d53cc8f58fd19c14537ac2
...
deploy_node() {
  ...
  ssh -o BatchMode=yes -o StrictHostKeyChecking=no -p "$port" "root@$host" \
    "set -e; \
     if pgrep -af '[s]glang.launch_server' >/dev/null; then \
       echo 'refusing to deploy r38 over a live SGLang process' >&2; exit 1; \
     fi; \
  ...
  scp -q -o BatchMode=yes -o StrictHostKeyChecking=no -P "$port" \
    ...
    "$experiment/master_config_e44_r38_get_prefix_reuse_enddepth288_netobs.yaml" \
  ...
  ssh -o BatchMode=yes -o StrictHostKeyChecking=no -p "$port" "root@$host" \
    "set -e; \
     ...
     source '$remote_experiment/e44_v5_perf_variant_20260718.sh' tier1_independent_005_netobs_enddepth288_get_prefix_reuse; \
     test \"\$E44_PERF_RUN_ID\" = e44_r38_get_prefix_reuse_enddepth288_netobs; \
     ...
     bash '$remote_experiment/install_release_e44_r38_get_prefix_reuse.sh' '$role' '$remote_release/$wheel_name' '$expected_pyo3_sha256'; \
