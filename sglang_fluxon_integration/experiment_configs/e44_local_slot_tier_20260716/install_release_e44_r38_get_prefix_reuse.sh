...
expected_commu_core_sha256="${4:-bfa6a32d991f6b6adf0f5175c07ed7da8290d1ed2a7ef4148b3a5f8b13452503}"
expected_rdma_probe_sha256="${5:-e925553e01d6f5f2754e667aeeac9bba8e7d829bf15136a95304f221e5dc5883}"
expected_cudart_sha256="${6:-}"
...
case "$role" in
  gpu)
    ...
    venv="${E44_INSTALL_VENV_GPU:-/storage/zth/sglang_l13_fluxon_v2/venv-fluxon-e44-r38-get-prefix-reuse-20260720}"
    python_version=3.10
    dependency_site=
    ;;
  cpu)
    python="${E44_INSTALL_CPU_PYTHON:-${PYTHON3_12:-$(command -v python3.12 || true)}}"
    if [ -z "$python" ]; then
      python=/usr/bin/python3.12
    fi
    venv="${E44_INSTALL_VENV_CPU:-/storage/mjq/sglang_fluxon/fluxon_cpu/venv-fluxon-e44-r38-get-prefix-reuse-20260720}"
    python_version="${E44_INSTALL_CPU_PYTHON_VERSION:-3.12}"
    if [ "${E44_INSTALL_CPU_DEPENDENCY_SITE+x}" = x ]; then
      dependency_site="$E44_INSTALL_CPU_DEPENDENCY_SITE"
...
if [[ ! "$python_version" =~ ^[0-9]+\.[0-9]+$ ]]; then
  echo "invalid Python major.minor version: $python_version" >&2
  exit 2
fi
if [ ! -x "$python" ]; then
...
test "$(sha256sum "$libs/libfluxon_commu_core.so" | awk '{print $1}')" = "$expected_commu_core_sha256"
test "$(sha256sum "$libs/libfluxon_rdma_probe.so" | awk '{print $1}')" = "$expected_rdma_probe_sha256"
if [ -n "$expected_cudart_sha256" ]; then
  test "$(sha256sum "$libs/libcudart.so.12" | awk '{print $1}')" = "$expected_cudart_sha256"
fi
