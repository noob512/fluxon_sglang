...
class UnifiedRadixCache(KVCacheEventMixin, BasePrefixCache):
    def __init__(
        ...
        dma_descriptor_cap_raw = os.getenv(
            "SGLANG_FLUXON_HOSTLESS_DMA_MAX_DESCRIPTORS_PER_CALL", "0"
        )
        try:
            dma_descriptor_cap = int(dma_descriptor_cap_raw)
        except ValueError as err:
            raise ValueError(
                "SGLANG_FLUXON_HOSTLESS_DMA_MAX_DESCRIPTORS_PER_CALL must be "
                f"a non-negative integer, got {dma_descriptor_cap_raw!r}"
            ) from err
        if dma_descriptor_cap < 0:
            raise ValueError(
                "SGLANG_FLUXON_HOSTLESS_DMA_MAX_DESCRIPTORS_PER_CALL must be "
                f"non-negative, got {dma_descriptor_cap}"
            )
        self._fluxon_hostless_dma_max_descriptors_per_call = dma_descriptor_cap
        if (
            self._fluxon_hostless_background_dma_submit_enabled
    ...
    def _build_fluxon_hostless_restore_kernel_plans(
        ...
        for operation in operations:
            if operation.plan_ptr is None:
                raise RuntimeError(
                    f"Fluxon layerwise restore lost its plan: node={operation.node_id}"
                )
    ...
    def _fluxon_hostless_dma_descriptor_slices(
        self,
        descriptor_count: int,
    ) -> tuple[tuple[int, int], ...]:
        if descriptor_count <= 0:
            raise RuntimeError(
                "Fluxon layer-batched DMA requires at least one descriptor"
            )
        cap = int(self._fluxon_hostless_dma_max_descriptors_per_call)
        if cap <= 0 or descriptor_count <= cap:
            return ((0, descriptor_count),)
        return tuple(
    ...
    def _enqueue_fluxon_hostless_layer_batch_dma(
        ...
        descriptor_count = int(src_ptrs.shape[1])
        descriptor_slices = self._fluxon_hostless_dma_descriptor_slices(
            descriptor_count
        )

        device_id = self._cuda_device_index()
        for layer_id in range(layer_num):
            for start, end in descriptor_slices:
                transfer_raw_h2d_batch(
                    dst_ptrs[layer_id, start:end],
                    src_ptrs[layer_id, start:end],
                    size_bytes[layer_id, start:end],
                    device_id,
                )
            producer_event.complete(layer_id)
    ...
    def _submit_fluxon_hostless_layer_batch_dma_background(
        ...
        try:
            ...
            logger.info(
                ...
                "max_descriptors_per_call=%d submit_cpu_ms=%.3f error=%s",
                len(operations),
                ...
                descriptor_count,
                submitted_dma_calls,
                int(self._fluxon_hostless_dma_max_descriptors_per_call),
                elapsed_ms,
                submit_error,
            )
    ...
    def _cancel_fluxon_hostless_get_start_handle(
        ...
        try:
            ...
            logger.warning(
                "Fluxon hostless get_start handle cancel failed: node=%s caller=%s error=%s",
                str(node_id),
                caller,
                exc,
            )
            ...
            try:
                ...
                if kv_tokens > 0:
                    ...
                    try:
                        ...
                        if kv_plan_ptr is None:
                            raise RuntimeError(
                                "Fluxon hostless KV restore requires a ready plan"
                            )
                ...
                if layerwise_async:
                    if queued_page_indices is None or kv_plan_ptr is None:
                        raise RuntimeError(
                            "Fluxon layerwise restore is missing its queued plan"
                        )
                    operation = _FluxonHostlessLayerwiseLoad(
                        backend=backend,
    ...
    def prefetch_from_storage(
        ...
        if self._is_fluxon_hostless_full_mode():
            ...
            if hash_values:
                ...
                try:
                    kv_handle = backend.get_start(
                        hash_values,
                        prefix_best_effort=True,
                        atomic_group_lens=atomic_group_lens,
                    )
                    kv_result = kv_handle.result
                    transferable_pages = int(
                        getattr(kv_result, "transferable_len", 0)
                    )
                except Exception as exc:
                    logger.warning(
                        "Fluxon hostless get_start failed; treating as cache miss: "
                    ...
                    self._cancel_fluxon_hostless_get_start_handle(
                        backend,
                        kv_handle,
                        "prefetch_get_start_error",
                    )
                    kv_handle = None
                    transferable_pages = 0
                    self._observe_fluxon_hostless_request(
                        req_id,
                        prefetch_decision="get_start_error",
                    )
                initial_start_ms = (
                    time.perf_counter() - initial_start_at
                ) * 1000.0
                self._observe_fluxon_hostless_request(
                    req_id,
                    requested_pages=requested_pages,
                    initial_transferable_pages=transferable_pages,
                    final_transferable_pages=transferable_pages,
                    initial_start_ms=initial_start_ms,
                )
                if transferable_pages < 0 or transferable_pages > requested_pages:
                    self._cancel_fluxon_hostless_get_start_handle(
                        backend,
                        kv_handle,
                        "prefetch_invalid_transferable_len",
                    )
                    kv_handle = None
                    self._finish_fluxon_hostless_request_observation(
                        req_id,
                        "invalid_initial_transferable_len",
                    )
                    raise RuntimeError(
                        "Fluxon hostless get_start returned invalid transferable_len: "
                        f"transferable={transferable_pages} requested={requested_pages}"
                    )
            if self.tp_world_size > 1:
                transfer_state = torch.tensor(
                    [transferable_pages, int(mamba_required_for_load_back)],
                    dtype=torch.int,
                )
                min_transfer_state = transfer_state.clone()
                max_transfer_state = transfer_state.clone()
                ...
                self._observe_fluxon_hostless_request(
                    req_id,
                    tp_min_pages=int(min_transfer_state[0].item()),
                    tp_max_pages=int(max_transfer_state[0].item()),
                )
                if int(min_transfer_state[1].item()) != int(max_transfer_state[1].item()):
                    logger.info(
                        "Fluxon hostless prefetch skipped by TP Mamba intent mismatch: "
                        "req=%s local_required=%d min_required=%d max_required=%d",
                        req_id,
                        int(mamba_required_for_load_back),
                        int(min_transfer_state[1].item()),
                        int(max_transfer_state[1].item()),
                    )
                    self._cancel_fluxon_hostless_get_start_handle(
                        backend,
                        kv_handle,
                        "prefetch_tp_mamba_intent_mismatch",
                    )
                    kv_handle = None
                    self._observe_fluxon_hostless_request(
                        req_id,
                    ...
                    return
                if int(min_transfer_state[0].item()) != int(max_transfer_state[0].item()):
                    initial_local_pages = transferable_pages
                    initial_min_pages = int(min_transfer_state[0].item())
                    initial_max_pages = int(max_transfer_state[0].item())
                    self._cancel_fluxon_hostless_get_start_handle(
                        backend,
                        kv_handle,
                    ...
                    if initial_min_pages <= 0:
                        logger.info(
                            "Fluxon hostless prefetch TP mismatch has no common prefix: "
                            "req=%s local_pages=%d min_pages=%d max_pages=%d",
                            req_id,
                            initial_local_pages,
                            initial_min_pages,
                            initial_max_pages,
                        ...
                        self._observe_fluxon_hostless_request(
                            req_id,
                            prefetch_decision="tp_no_common_prefix",
                            final_transferable_pages=0,
                        )
                        return
                    retry_group_lens: list[int] = []
                    retry_group_pages = 0
                    source_group_lens = atomic_group_lens or [1] * requested_pages
                    for group_len in source_group_lens:
                        if retry_group_pages + group_len > initial_min_pages:
                            break
                        retry_group_lens.append(group_len)
                        retry_group_pages += group_len
                    if retry_group_pages != initial_min_pages:
                        self._finish_fluxon_hostless_request_observation(
                            req_id,
                            "tp_common_prefix_split_atomic_group",
                        )
                        raise RuntimeError(
                            "Fluxon TP common transferable prefix split an atomic group: "
                            f"req={req_id} common={initial_min_pages} "
                            f"group_pages={retry_group_pages} groups={source_group_lens}"
                        )
                    retry_start_at = time.perf_counter()
                    try:
                        kv_handle = backend.get_start(
                            hash_values[:initial_min_pages],
                            prefix_best_effort=True,
                            atomic_group_lens=retry_group_lens,
                        )
                        kv_result = kv_handle.result
                        transferable_pages = int(
                            getattr(kv_result, "transferable_len", 0)
                        ...
                        logger.warning(
                            ...
                            "treating as cache miss: req=%s requested_pages=%d error=%s",
                            req_id,
                            initial_min_pages,
                            exc,
                        )
                        self._cancel_fluxon_hostless_get_start_handle(
                            backend,
                            kv_handle,
                            "prefetch_tp_common_prefix_retry_error",
                        )
                        kv_handle = None
                        transferable_pages = 0
                        self._observe_fluxon_hostless_request(
                            req_id,
                            prefetch_decision="tp_common_prefix_retry_error",
                        )
                    retry_start_ms = (
                        time.perf_counter() - retry_start_at
                    ) * 1000.0
                    self._observe_fluxon_hostless_request(
                        req_id,
                        retry_count=1,
                        retry_start_ms=retry_start_ms,
                        final_transferable_pages=transferable_pages,
                    )
                    if transferable_pages < 0 or transferable_pages > initial_min_pages:
                        self._cancel_fluxon_hostless_get_start_handle(
                            backend,
                            kv_handle,
                            "prefetch_tp_common_prefix_invalid_retry",
                        )
                        kv_handle = None
                        self._finish_fluxon_hostless_request_observation(
                            req_id,
                            "invalid_retry_transferable_len",
                        )
                        raise RuntimeError(
                            "Fluxon TP common-prefix retry returned invalid length: "
                            f"req={req_id} transferable={transferable_pages} "
                            f"requested={initial_min_pages}"
                        )
                    retry_transfer_state = torch.tensor(
                        transferable_pages,
                        dtype=torch.int,
                    )
                    retry_min_pages = retry_transfer_state.clone()
                    retry_max_pages = retry_transfer_state.clone()
                    torch.distributed.all_reduce(
                        retry_min_pages,
                        op=torch.distributed.ReduceOp.MIN,
                        group=self.tp_group,
                    ...
                    if int(retry_min_pages.item()) != int(retry_max_pages.item()):
                        logger.info(
                            "Fluxon hostless prefetch TP common-prefix retry still mismatched: "
                            "req=%s local_pages=%d min_pages=%d max_pages=%d",
                            req_id,
                            transferable_pages,
                            int(retry_min_pages.item()),
                            int(retry_max_pages.item()),
                        )
                        self._cancel_fluxon_hostless_get_start_handle(
                            backend,
                            kv_handle,
                            "prefetch_tp_common_prefix_retry_mismatch",
                        )
                        kv_handle = None
                        self._observe_fluxon_hostless_request(
                            req_id,
                            prefetch_decision="tp_retry_still_mismatched",
                        )
                        return
                    logger.info(
                        "Fluxon hostless prefetch converged to TP common prefix: "
                        "req=%s initial_local=%d initial_min=%d initial_max=%d "
                        "retry_pages=%d",
                        req_id,
                        initial_local_pages,
                        initial_min_pages,
                    ...
                    if observation.get("prefetch_decision") != (
                        "tp_common_prefix_retry_error"
                    ):
                        observation[
                            "prefetch_decision"
                        ] = "tp_common_prefix_converged"
                    observation.update(
                        tp_min_pages=int(retry_min_pages.item()),
                        tp_max_pages=int(retry_max_pages.item()),
                        final_transferable_pages=transferable_pages,
                    )
            ...
            if hash_values:
                if mamba_required_for_load_back and transferable_pages != requested_pages:
                    self._cancel_fluxon_hostless_get_start_handle(
                        backend,
                        kv_handle,
                        "prefetch_mamba_requires_full_kv_prefix",
                    )
                    kv_handle = None
                    hash_values = []
                    transferable_pages = 0
            ...
            self._observe_fluxon_hostless_request(
                ...
                prefetch_decision=(
                    ...
                    if previous_decision == "tp_common_prefix_converged"
                    else "submitted"
                ),
                requested_pages=requested_pages,
                final_transferable_pages=transferable_pages,
            ...
            logger.info(
                "HiCache prefetch submitted: req=%s tokens=%d mode=fluxon_hostless_full pages=%d transferable=%d mamba=%s occupied=%d",
                req_id,
                total_tokens,
                requested_pages,
                transferable_pages,
    ...
    def check_prefetch_progress(self, req_id: str) -> bool:
        ...
        if isinstance(operation, _FluxonHostlessPrefetchOperation):
            ...
            logger.info(
                "HiCache prefetch success req=%s mode=fluxon_hostless_full completed_local=%d completed_synced=%d occupied=%d",
                req_id,
                local_completed_tokens,
                min_completed_tokens,
                self.cache_controller.prefetch_tokens_occupied,
