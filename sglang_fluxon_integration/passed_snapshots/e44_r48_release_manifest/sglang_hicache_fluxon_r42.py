...
class _FluxonBatchWarmFuture:
    ...
    def is_waiting(self) -> bool:
        return not self._batch_future.done()

    def wait(self) -> Any:
        try:
            batch_results = self._batch_future.result()
...
class _FluxonGpuStagingLease:
    ...
    def release(self) -> None:
        ...
        self.destinations = ()
        self.value_ptrs = ()
        self._pool._release_slots(slots)
...
class _FluxonGpuStagingPool:
    ...
    def try_reserve(self, page_count: int) -> Optional[_FluxonGpuStagingLease]:
        if page_count <= 0:
            return None
        with self._lock:
            if self._closed or page_count > len(self._free_slots):
                return None
            slots = [self._free_slots.pop() for _ in range(page_count)]
        return _FluxonGpuStagingLease(self, slots)
    ...
    def close(self) -> None:
        with self._lock:
            if self._closed:
                return
            live_slots = self.slot_count - len(self._free_slots)
            if live_slots != 0:
                raise RuntimeError(
                    f"GPU staging close has live slots: {live_slots}/{self.slot_count}"
                )
            self._closed = True
...
def _import_fluxon_symbols() -> tuple[Any, Any, Any]:
    try:
...
class HiCacheFluxon(HiCacheStorage):
    ...
    def __init__(self, storage_config: HiCacheStorageConfig):
        ...
        FluxonKvClientConfig, new_store, PutOptionalArgs = _import_fluxon_symbols()
    ...
    def _submit_warm_get_batch(self, storage_keys: list[str]) -> int:
        ...
        with self._warm_lock:
            if not self._enable_warm_get:
                return 0
            remaining = self._warm_limit - len(self._warm_futures)
            if remaining <= 0:
                return 0
            for storage_key in storage_keys:
    ...
    def local_fast_put_start(
        ...
        plan_ptr = self._call_local_fast_put_start(
            ...
            opts=self._put_opts_for_replica_task_mask(
                admission_mask, atomic_group_lens
            ),
            make_replica_task=any(admission_mask),
            admission_mask=admission_mask,
            atomic_group_lens=atomic_group_lens,
    ...
    def try_reserve_gpu_direct_staging(
        self,
        page_count: int,
    ) -> Optional[_FluxonGpuStagingLease]:
        pool = self._gpu_direct_staging_pool
        if pool is None:
            return None
        return pool.try_reserve(int(page_count))
    ...
    def get_transfer_gpu(
        ...
        *,
        ...
        logger.info(
            "Fluxon get_transfer_gpu success: consume_prefix_len=%s duration_ms=%.3f",
            consume_prefix_len,
            (time.perf_counter() - start) * 1000.0,
        )
    ...
    def view_value_ptrs(
        ...
        if value_count != expected_count:
            raise RuntimeError(
                "Fluxon plan value count mismatch: "
                f"expected={expected_count} got={value_count}"
            )
        values = (ctypes.c_uint64 * value_count).from_address(plan_ptr + 16)
