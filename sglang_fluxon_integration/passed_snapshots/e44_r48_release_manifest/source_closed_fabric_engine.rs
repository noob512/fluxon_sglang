...
use crate::{
    GdrFlag,
    api::{GdrCounter, UvmWatcherId},
    cuda_support::UvmWatcherCall,
};
...
fn worker_for_device<'a, T>(workers: &'a BTreeMap<u8, T>, device: &Device) -> Option<&'a T> {
    match device {
        Device::Host => workers.first_key_value().map(|(_, worker)| worker),
        Device::Cuda(CudaDeviceId(device_id)) => workers.get(device_id).or_else(|| {
            // A single-worker engine owns one RDMA domain group, not one CUDA context.  PPLX
            // deliberately builds that shape for each external process and keeps the real CUDA
            // device in the MemoryRegion mapping.  Therefore a TP rank whose physical device is
            // not worker key 0 must still use the sole domain worker.  Multi-worker engines keep
            // strict device-to-worker matching so topology mistakes remain visible.
            (workers.len() == 1)
                .then(|| workers.first_key_value().map(|(_, worker)| worker))
                .flatten()
        }),
    }
}
...
impl FabricEngine {
    ...
    pub fn unregister_memory(&self, ptr: NonNull<c_void>) -> Result<()> {
        ...
        let worker = self.get_worker_by_mr(handle)?;
        let (tx, rx) = oneshot::channel();
    ...
    pub fn acquire_uvm_watcher(&self) -> Result<UvmWatcherId> {
        ...
        worker
            ...
            .send(cmd)
            .map_err(|_| FabricLibError::Custom("Worker is down"))?;
        let maybe: Option<UvmWatcherId> = rx
            .recv()
            .map_err(|_| FabricLibError::Custom("Worker is down"))?;
    ...
    fn get_worker(&self, device: &Device) -> Result<&WorkerContext> {
        worker_for_device(&self.workers, device).ok_or(FabricLibError::Custom("Worker not found"))
    }
...
mod tests {
    ...
    #[test]
    fn single_domain_worker_accepts_nonzero_cuda_device() {
        let workers = BTreeMap::from([(0, "only")]);

        assert_eq!(
            worker_for_device(&workers, &Device::Cuda(CudaDeviceId(1))),
            Some(&"only")
        );
    }

    #[test]
    fn multi_worker_engine_keeps_cuda_device_matching_strict() {
        let workers = BTreeMap::from([(0, "zero"), (1, "one")]);
        ...
        assert_eq!(
            worker_for_device(&workers, &Device::Cuda(CudaDeviceId(2))),
            None
        );
    }
}
