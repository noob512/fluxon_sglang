...
class HostKVCache(abc.ABC):
    ...
    def __init__(
        ...
        # Fluxon is the sole L2/L3 data authority. The SGLang HostKV pool keeps
        # logical slot metadata only and must never materialize a second KV tier.
        metadata_only_requested = str(allocator_type).lower() == "fluxon"
        if metadata_only_requested and not isinstance(device_pool, MHATokenToKVPool):
            raise ValueError(
                "Fluxon metadata-only host pool currently supports only "
                "MHATokenToKVPool; refusing to silently allocate or emulate "
                f"host KV data for {type(device_pool).__name__}"
            )
        self.metadata_only = metadata_only_requested
        ...
        if self.metadata_only:
            logger.warning(
                "Fluxon metadata-only HostKV pool enabled: logical_tokens=%d "
                "logical_bytes=%.2f GB materialized_pages=1; host KV data "
                "access is forbidden and Fluxon is the sole L2/L3 authority.",
                self.size,
                requested_bytes / 1e9,
            )
        else:
    ...
    def _require_materialized_host_data(self, api_name: str) -> None:
        if self.metadata_only:
            raise RuntimeError(
                ...
                "mode; the unified radix path must use storage_backed hostless I/O"
            )
