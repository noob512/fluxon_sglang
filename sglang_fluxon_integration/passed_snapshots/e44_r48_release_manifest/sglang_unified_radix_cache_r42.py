...
class _FluxonHostlessLayerwiseLoad:
    def __init__(
        ...
        if (plan_ptr is None) == (gpu_staging_lease is None):
            raise RuntimeError(
                "Fluxon layerwise restore requires exactly one CPU plan or GPU staging lease"
            )
...
class UnifiedRadixCache(KVCacheEventMixin, BasePrefixCache):
    ...
    def _build_fluxon_gpu_staging_plan_blob(
        self,
        staging_lease: Any,
        expected_pages: int,
    ) -> torch.Tensor:
        if staging_lease.released:
            raise RuntimeError("Fluxon GPU staging lease was released before restore")
        value_ptrs = tuple(int(ptr) for ptr in staging_lease.value_ptrs)
        if len(value_ptrs) != expected_pages:
            raise RuntimeError(
                "Fluxon GPU staging restore page mismatch: "
                f"lease={len(value_ptrs)} expected={expected_pages}"
            )
        ...
        plan_blob[0] = _FLUXON_PLAN_BLOB_MAGIC
        plan_blob[1] = expected_pages
    ...
    def _build_fluxon_hostless_restore_kernel_plans(
        ...
        for operation in operations:
            if (
                operation.plan_ptr is None
                and operation.gpu_staging_lease is None
            ):
                raise RuntimeError(
    ...
    def _cancel_fluxon_hostless_get_start_handle(
        ...
        try:
            ...
            logger.warning(
                ...
                "node=%s caller=%s gpu_direct=%s error=%s",
                str(node_id),
                caller,
                gpu_direct,
                exc,
            ...
            try:
                ...
                if kv_tokens > 0:
                    ...
                    if self._fluxon_hostless_ready_operation_covers_kv(
                        ...
                        if ready_prefetch_operation.gpu_direct:
                            ...
                            staging_lease.trim_after_transfer(
                                len(kv_batch_hash_values)
                            )
                ...
                if kv_tokens > 0:
                    ...
                    try:
                        ...
                        if (
                            kv_plan_ptr is None
                            and kv_gpu_staging_lease is None
                        ):
                            raise RuntimeError(
                        ...
                        if layerwise_async:
                            ...
                            restore_plan_ptr = kv_plan_ptr
                            if kv_gpu_staging_lease is not None:
                                gpu_plan_blob = (
                                    self._build_fluxon_gpu_staging_plan_blob(
                                        kv_gpu_staging_lease,
                                        int(page_indices.numel()),
                                    )
                                )
                                restore_plan_ptr = int(gpu_plan_blob.data_ptr())
                                raw_h2d_state.retain(gpu_plan_blob)
                                raw_h2d_state.add_finalizer(
                                    kv_gpu_staging_lease.release
                                )
                                kv_gpu_staging_lease = None
                ...
                if layerwise_async:
                    if queued_page_indices is None or (
                        kv_plan_ptr is None
                        and kv_gpu_staging_lease is None
                    ):
                        raise RuntimeError(
                    ...
                    if kv_gpu_staging_lease is not None:
                        value_ptrs = tuple(kv_gpu_staging_lease.value_ptrs)
                    else:
    ...
    def prefetch_from_storage(
        ...
        if self._is_fluxon_hostless_full_mode():
            ...
            def cancel_kv_handle(caller: str) -> None:
                nonlocal kv_handle, gpu_staging_lease
                self._cancel_fluxon_hostless_get_start_handle(
                    backend,
                    kv_handle,
                    caller,
            ...
            if hash_values and not mamba_required_for_load_back:
                gpu_staging_lease = backend.try_reserve_gpu_direct_staging(
                    len(hash_values)
                )
            ...
            gpu_reservation_consistent = (
                ...
                and int(gpu_reservation_min[1].item())
                == int(gpu_reservation_max[1].item())
                and int(gpu_reservation_max[2].item()) == 0
            )
            ...
            if hash_values:
                ...
                try:
                    if kv_gpu_direct:
                        kv_handle = backend.get_start_gpu(
                            hash_values,
                            gpu_staging_lease,
                    ...
                    cancel_kv_handle("prefetch_get_start_error")
                    transferable_pages = 0
                    self._observe_fluxon_hostless_request(
                        req_id,
                ...
                if transferable_pages < 0 or transferable_pages > requested_pages:
                    cancel_kv_handle("prefetch_invalid_transferable_len")
                    self._finish_fluxon_hostless_request_observation(
                        req_id,
            ...
            if self.tp_world_size > 1:
                ...
                if int(min_transfer_state[0].item()) != int(max_transfer_state[0].item()):
                    ...
                    if initial_min_pages <= 0:
                        cancel_kv_handle(
                            "prefetch_tp_transferable_page_mismatch_zero_common"
                        )
                        logger.info(
                        ...
                        return
                    common_group_pages = 0
                    source_group_lens = atomic_group_lens or [1] * requested_pages
                    for group_len in source_group_lens:
                        if common_group_pages + group_len > initial_min_pages:
                            break
                        common_group_pages += group_len
                    if common_group_pages != initial_min_pages:
                        cancel_kv_handle(
                            "prefetch_tp_common_prefix_split_atomic_group"
                        )
                        self._finish_fluxon_hostless_request_observation(
                            req_id,
                        ...
                        raise RuntimeError(
                            ...
                            f"group_pages={common_group_pages} groups={source_group_lens}"
                        )
                    transferable_pages = initial_min_pages
                    self._observe_fluxon_hostless_request(
                        req_id,
                        retry_count=0,
                        retry_start_ms=0.0,
                    ...
                    observation.update(
                        tp_min_pages=initial_min_pages,
                        tp_max_pages=initial_min_pages,
                        final_transferable_pages=transferable_pages,
                    )
